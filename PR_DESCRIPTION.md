...
- `backend/server.js`: افزودنِ کامنتِ توجیهیِ صریح (anti-pattern
  ...
  branchِ خطا وابسته بود. terminal catch-all (که بعد از `applySecurity()`
  ثبت می‌شود) همهٔ خطاها را به یک پاسخِ JSONِ یکنواخت تبدیل می‌کند. ground
  truthِ این قرارداد در `tests/test_error_handling.py` و
...
- [x] `pytest` کاملِ ریپو — ۲۶ تست سبز
