---
task_id: f07fd28c-d422-435a-bcd1-ea265f5d9b44
...
created_at: '2026-06-06T07:59:11.460717+00:00'
updated_at: '2026-06-13T08:13:37.635071+00:00'
target_files:
...
## 📋 شرح
در `backend/app/notifications.py` یک notification call پیدا شد که `silent=` صریح تعیین نکرده. event_type: `?`.
...
---
[scan #2 at 2026-06-06T07:59:11.507156+00:00]
## 📋 شرح
در `backend/app/notifications.py` یک `notify_event` call پیدا شد که اولین پارامتر (event_type) رشته‌ای ساده/مبهم است یا خالی.
...
- علت:
---
[scan #3 at 2026-06-13T08:13:37.531738+00:00]
## 📋 شرح
در `backend/app/notifications.py` یک notification call پیدا شد که `silent=` صریح تعیین نکرده. event_type: `?`.

## 🤔 چرا مهم است
silent flag حیاتی است — تفاوت بین «کاربر را disturb کن (critical)» و «در پس‌زمینه لاگ کن (routine)». بدون این، behavior به default وابسته است که در آینده ممکن است
---
[scan #4 at 2026-06-13T08:13:37.558467+00:00]
## 📋 شرح
در `backend/app/notifications.py` یک `notify_event` call پیدا شد که اولین پارامتر (event_type) رشته‌ای ساده/مبهم است یا خالی.
...
- علت:
---
[scan #5 at 2026-06-13T08:13:37.608500+00:00]
متغیر `context` در این فراخوانی عمومی است و محتوای آن مشخص نیست. این امر باعث می‌شود نتوانیم اطمینان حاصل کنیم که عنوان، متن توضیحی و لینک‌های عملیاتی (action links) برای یک کپشن کامل و مفید فراهم شده‌اند. برای قالب‌بندی موثر، `context` باید یک دیکشنری ساختاریافته با جزئیات مشخص باشد.

🛠 پیشنهاد: بر
---
[scan #6 at 2026-06-13T08:13:37.635059+00:00]
محتوای متغیر `context` نامشخص است. بدون دانستن اینکه آیا این متغیر شامل کلیدهایی مانند `task_id` یا `scan_id` است، نمی‌توانیم تأیید کنیم که پیوست‌های صحیح (مانند `prompt.md` برای `task` یا `bundle.pdf` برای `scan`) می‌توانند تولید شوند یا به درستی نگاشت شده‌اند.

🛠 پیشنهاد: اطمینان حاصل کنید که `con
...
- `backend/app/notifications.py:124`
...
- `backend/app/monitoring.py` — این فایل `notifications.py` را import می‌کند (caller)
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile backend/app/notifications.py`
- `ruff check backend/app/notifications.py`
- `pytest -x`
