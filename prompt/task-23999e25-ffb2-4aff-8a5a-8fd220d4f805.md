---
task_id: 23999e25-ffb2-4aff-8a5a-8fd220d4f805
title: '[منطق] Untested ValueError from Vocabulary Store'
type: logic_audit
priority: high
...
created_at: '2026-06-13T08:11:29.976474+00:00'
updated_at: '2026-06-13T08:18:09.905836+00:00'
---

# [منطق] Untested ValueError from Vocabulary Store
...
The `_vocabulary.py` component explicitly states that it can raise a `ValueError`. However, the `test_vocabulary.py` component, which is responsible for verifying the store's functionality, only explicitly mentions testing for `DuplicateWordError` and `WordNotFoundError`. This indicates a potential gap in test coverage for scenarios that should result in a `ValueError`.

## 💥 پیامد (impact)
If `_vocabulary.py` raises `ValueError` under specific conditions (e.g., invalid input data, malformed parameters), these edge cases might not be caught by the unit tests. This could lead to unexpected runtime errors or incorrect behavior in components that rely on this vocabulary store, even if it'

## 🛠 پیشنهاد رفع اولیه
Add specific unit tests in `test_vocabulary.py` to cover scenarios that are expected to raise `ValueError` in `_vocabulary.py`. Additionally, clarify in `_vocabulary.py`'s description under what specific conditions a `ValueError` is raised.
...
### Step 1: Clarify ValueError conditions in `_vocabulary.py`'s description
**Status:** `pending` (0%)
**Scope:** این مرحله شامل به‌روزرسانی داک‌استرینگ یا کامنت‌های مربوطه در کامپوننت `_vocabulary.py` است تا شرایط دقیقی که تحت آن انتظار می‌رود یک `ValueError` ایجاد شود، به وضوح تشریح گردد. هدف اصلی این است که مستندات واضح و بدون ابهامی برای توسعه‌دهندگان و تست‌کنندگان فراهم شود، بدون اینکه هیچ تغییری در کد عملکردی یا اضافه کردن تست‌های جدید صورت گیرد.
**Excerpt:**
```
Add specific unit tests in `test_vocabulary.py` to cover scenarios that are expected to raise `ValueError` in `_vocabulary.py`. Additionally, clarify in `_vocabulary.py`'s description under what specific conditions a `ValueError` is raised.
```

### Step 2: Add specific unit tests for `ValueError` in `test_vocabulary.py`
**Status:** `pending` (0%)
**Scope:** این مرحله بر ایجاد موارد تست واحد جدید در فایل `test_vocabulary.py` تمرکز دارد. این تست‌ها به طور خاص سناریوهایی را هدف قرار می‌دهند که انتظار می‌رود `ValueError` را در کامپوننت `_vocabulary.py` ایجاد کنند. تست‌ها باید تأیید کنند که `ValueError` تحت شرایط مستند شده ایجاد می‌شود و پوشش تست کاملی را برای این نوع استثنا تضمین می‌کند.
**Excerpt:**
```
Add specific unit tests in `test_vocabulary.py` to cover scenarios that are expected to raise `ValueError` in `_vocabulary.py`. However, the `test_vocabulary.py` component, which is responsible for verifying the store's functionality, only explicitly mentions testing for `DuplicateWordError` and `WordNotFoundError`. This indicates a potential gap in test coverage for scenarios that should result in a `ValueError`.
```
