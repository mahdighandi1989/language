---
task_id: db1d8c47-e61b-4556-be2a-b43691ad7cbb
...
updated_at: '2026-06-13T08:12:02.899445+00:00'
target_files:
...
The `chatFeedbackInstruction` prompt expects the AI to append specific markers (`[FEEDBACK:correct]`, `[FEEDBACK:incorrect]`, `[FEEDBACK:partial]`) to its responses. There is no visible code to validate if the AI actually includes these markers, if they are correctly formatted, or to handle cases where they are missing or malformed. Relying on the AI to perfectly follow such instructions without c

📁 file: frontend/src/components/App.jsx (line 66)

🎯 پیشنهاد: این الگو معمولاً منطق سیستم را در شرایط لبه می‌شکند.
---
[scan #2 at 2026-06-06T07:57:45.069181+00:00]
## 📋 شرح
یک دکمه/کنترل UI در فایل `frontend/src/components/App.jsx` پیدا شد که هیچ event handler معنادار به آن متصل نیست (onClick، onChange، form submit، router push، یا API call شناسایی نشد).

## 🔍 جزئیات
- label/متن دکمه: `setIsAssistantOpen(true)} className="fixed bottom-6 right-6 bg-teal-500 tex
---
[scan #3 at 2026-06-06T07:57:45.079611+00:00]
## 📋 شرح
یک دکمه/کنترل UI در فایل `frontend/src/components/App.jsx` پیدا شد که هیچ event handler معنادار به آن متصل نیست (onClick، onChange، form submit، router push، یا API call شناسایی نشد).
...
---
[scan #4 at 2026-06-06T07:57:45.090314+00:00]
## 📋 شرح
یک دکمه/کنترل UI در فایل `frontend/src/components/App.jsx` پیدا شد که هیچ event handler معنادار به آن متصل نیست (onClick، onChange، form submit، router push، یا API call شناسایی نشد).

## 🔍 جزئیات
- label/متن دکمه: `{ navigateTo(view); setMobileOpen(false); }} className={`w-full flex items-
---
[scan #5 at 2026-06-06T07:57:45.099691+00:00]
## 📋 شرح
یک دکمه/کنترل UI در فایل `frontend/src/components/App.jsx` پیدا شد که هیچ event handler معنادار به آن متصل نیست (onClick، onChange، form submit، router push، یا API call شناسایی نشد).

## 🔍 جزئیات
- label/متن دکمه: `setMobileOpen(o => !o)} className="md:hidden p-2 rounded-lg hover:bg-slate-
---
[scan #6 at 2026-06-06T07:57:45.108688+00:00]
## 📋 شرح
یک دکمه/کنترل UI در فایل `frontend/src/components/App.jsx` پیدا شد که هیچ event handler معنادار به آن متصل نیست (onClick، onChange، form submit، router push، یا API call شناسایی نشد).

## 🔍 جزئیات
- label/متن دکمه: `{ onSearchClick(); setMobileOpen(false); }} className="w-full flex items-ce
...
- `frontend/src/components/App.jsx:66`
