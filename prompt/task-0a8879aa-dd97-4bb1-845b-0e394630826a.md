---
task_id: 0a8879aa-dd97-4bb1-845b-0e394630826a
title: Telegram webhook route lacks the rate limiter and validation applied to /api/* router
type: security
priority: medium
...
created_at: '2026-06-13T08:12:02.720477+00:00'
updated_at: '2026-06-13T08:19:40.610027+00:00'
target_files:
- backend/server.js
- backend/services/telegram/index.js
---

# Telegram webhook route lacks the rate limiter and validation applied to /api/* router

## Raw Idea

در server.js ابتدا `attachTelegram(app, ...)` فراخوانی می‌شود (خط 87) و سپس rate limiter روی `/api` نصب می‌شود (خط 95). routeهای تلگرام (`/api/telegram/webhook`, `/api/telegram/link`, `/api/telegram/status`) داخل attachTelegram روی `app` مستقیماً ثبت می‌شوند پیش از نصب `generalLimiter`، بنابراین webhook و link از rate limiting محروم‌اند. webhook یک endpoint عمومی است که هرکس می‌تواند POST کند؛ اگرچه با X-Telegram-Bot-Api-Secret-Token محافظت می‌شود، نبود rate limit امکان DoS/brute-force روی secret را می‌دهد. همچنین `/api/telegram/link` بدنه‌ی کاربر را بدون zod validation می‌خواند.
...
- `backend/server.js:86-95` — `attachTelegram wiring` — comment صریحاً اذعان می‌کند تلگرام قبل از limiter ثبت می‌شود
  ```jsx
  ...
  ```
- `backend/services/telegram/index.js:89-119` — `attachTelegram` — webhook بدون limiter؛ link بدون zod validate
  ```jsx
...
Express 4, express-rate-limit 8, Telegram Bot webhook

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/middleware/rateLimiter.js` (سطر 15) — generalLimiter/authLimiter که باید روی این routeها اعمال شود
- `backend/middleware/validate.js` (سطر 6) — middleware validate برای schema بدنه‌ی link
...
## 🌐 نقشهٔ وابستگی‌ها
attachTelegram از server.js فراخوانی می‌شود و routeها را روی app نصب می‌کند؛ جابه‌جایی ترتیب نصب limiter اثر سراسری روی محافظت همه‌ی routeهای /api دارد.
...
- [ ] درخواست‌های مکرر به /api/telegram/webhook پس از حد مجاز با 429 پاسخ می‌گیرند
- [ ] /api/telegram/link بدنه‌ی نامعتبر (code غیررشته‌ای) را با 400 رد می‌کند
- [ ] تست‌های telegram موجود همچنان pass می‌شوند
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. ثبت routeهای تلگرام را بعد از نصب `generalLimiter` منتقل کن، یا یک limiter اختصاصی به webhook/link اضافه کن. ترتیب در server.js را اصلاح کن: ابتدا `app.use('/api', generalLimiter)` سپس `attachTelegram`. روی `/api/telegram/link` یک schema validation برای `code` اضافه کن.

## 💡 نمونه‌های قبل/بعد
**limiter before telegram**
...
## 🧪 دستورات اعتبارسنجی
- `node backend/tests/test_telegram_service.js`
- `npm run test --workspace backend`

## ⚠️ ریسک‌ها و موارد احتیاط
تغییر ترتیب ممکن است status endpoint سبک را هم زیر limiter ببرد؛ در صورت نیاز status را مستثنا کن
...
1. درخواست‌های مکرر به /api/telegram/webhook پس از حد مجاز با 429 پاسخ می‌گیرند _(verify: api_response)_
2. /api/telegram/link بدنه‌ی نامعتبر (code غیررشته‌ای) را با 400 رد می‌کند _(verify: api_response)_
3. تست‌های telegram موجود همچنان pass می‌شوند _(verify: backend_test)_
