---
task_id: 477663a8-d3a3-44f2-a739-58d114dd2d39
title: فایل بحرانی backend/services/analysisService.js (orchestrator اصلی AI pipeline) هیچ تست واحد مستقیمی ندارد
type: test
priority: critical
...
created_at: '2026-06-13T08:07:28.870975+00:00'
updated_at: '2026-06-13T08:15:12.550748+00:00'
target_files:
- backend/services/analysisService.js
---

# فایل بحرانی backend/services/analysisService.js (orchestrator اصلی AI pipeline) هیچ تست واحد مستقیمی ندارد

## Raw Idea

فایل analysisService.js قلب پایپ‌لاین AI/LLM است؛ orchestration کاملِ تحلیل چندوجهی (PDF/audio/video/image/text)، منطق chunking برای فایل‌های بزرگ، File API برای فایل‌های >15MB، استخراج صوت/فریم با ffmpeg برای ویدئوهای >50MB و merge نتایج همگی اینجا اتفاق می‌افتند. علی‌رغم پیچیدگی بالا و branching زیاد (mimeType مختلف، isLargeFile، PDF chunking، error handling per-file) هیچ فایل تست واحدی به نام analysisService.test.js در backend/tests/ وجود ندارد. تست‌های موجود تحت tests/integration/test_ai_llm_pipeline.py و tests/backend/ai_llm/ فقط ground-truth پایتونی (pipeline.py) را پوشش می‌دهند، نه پیاده‌سازی واقعی JS. این یعنی منطق انتخاب مسیر بر اساس mimeType، آستانه‌های 15MB/50MB/50000 chars و cleanup مسیرهای temp بدون رگرسیون‌گاردِ مستقیم باقی مانده‌اند. شواهد: ساختار شاخه‌ای گسترده در analyzeUploads (خطوط 41-362) و عدم وجود فایل تست متناظر در فهرست backend/tests/.
...
- `backend/services/analysisService.js:22-362` — `analyzeUploads` — تابع orchestrator اصلی با branching سنگین که هیچ تست مستقیمی ندارد
  ```jsx
  export async function analyzeUploads({ files = [], textContent = '', userInstructions = '' }) {
    ...
    for (const file of files) {
      ...
      ... if (mimeType === 'application/pdf') ... else if (mimeType.startsWith('audio/')) ...
  ```
...
Node/Express ESM + Vitest + Gemini REST/File API + ffmpeg

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/services/geminiService.js` — وابستگی اصلی که باید mock شود (analyzeWithGemini, uploadToGeminiFileAPI, deleteGeminiFile)
- `backend/services/pdfService.js` — extractPdfText برای مسیر PDF فراخوانی می‌شود
- `backend/services/videoService.js` — extractAudioFromVideo و extractKeyFrames برای ویدئوهای بزرگ
- `backend/utils/fileCleanup.js` — cleanupTempFiles باید همیشه فراخوانی شود — باید تست شود
- `backend/tests/exercises.test.js` — الگوی سبک تست Vitest موجود برای service-layer
- `backend/services/prompts.js` — `analysisService.js` این فایل را import می‌کند
- `backend/utils/chunking.js` — `analysisService.js` این فایل را import می‌کند
- `backend/controllers/analysisController.js` — این فایل `analysisService.js` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
analysisService مصرف‌کنندهٔ geminiService/pdfService/videoService است و خودش توسط controllers/analysisController.js (روت /api/analyze-files) فراخوانی می‌شود؛ هر رگرسیون در آن مستقیماً قابلیت اصلی تحلیل فایل کاربر را خراب می‌کند.
...
- [ ] فایل backend/tests/analysisService.test.js ایجاد شود و در اجرای vitest سبز باشد
- [ ] تست مسیر 400 (بدون فایل/متن) وجود داشته باشد
- [ ] حداقل یک تست برای chunking PDF (آستانهٔ 50000) و یک تست برای مسیر isLargeFile صوت با mock File API وجود داشته باشد
- [ ] تستی که اطمینان می‌دهد cleanupTempFiles در مسیر موفق و خطا فراخوانی می‌شود اضافه شود
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. یک فایل backend/tests/analysisService.test.js (Vitest، هم‌سبک با exercises.test.js و flashcardService.test.js موجود) اضافه کن که geminiService و pdfService و videoService و fileCleanup را mock کند و این مسیرها را پوشش دهد: (۱) خطای 400 وقتی نه فایل و نه textContent وجود دارد؛ (۲) مسیر PDF با و بدون chunking (آستانهٔ 50000 char)؛ (۳) مسیر audio با isLargeFile=true (File API + deleteGeminiFile در finally) و false (inline)؛ (۴) مسیر image و text؛ (۵) ادامه (continue) هنگام خطای خواندن فایل؛ (۶) فراخوانی cleanupTempFiles حتی هنگام خطا. برای تزریق mock، importهای geminiService را با vi.mock جایگزین کن.
...
_قبل:_
```
// هیچ تستی برای analyzeUploads وجود ندارد
```

_بعد:_
```
it('throws 400 when no files and no text', async () => {
  await expect(analyzeUploads({})).rejects.toMatchObject({ status: 400 });
});
```
...
## 🧪 دستورات اعتبارسنجی
- `npx vitest run backend/tests/analysisService.test.js`
- `npm run test --workspace backend`

## ⚠️ ریسک‌ها و موارد احتیاط
نیاز به mock دقیق ماژول‌های ESM با vi.mock؛ احتمال flakiness اگر mockها timing/finally را پوشش ندهند
...
1. فایل backend/tests/analysisService.test.js ایجاد شود و در اجرای vitest سبز باشد _(verify: static)_
2. تست مسیر 400 (بدون فایل/متن) وجود داشته باشد _(verify: static)_
3. حداقل یک تست برای chunking PDF (آستانهٔ 50000) و یک تست برای مسیر isLargeFile صوت با mock File API وجود داشته باشد _(verify: static)_
4. تستی که اطمینان می‌دهد cleanupTempFiles در مسیر موفق و خطا فراخوانی می‌شود اضافه شود _(verify: static)_
