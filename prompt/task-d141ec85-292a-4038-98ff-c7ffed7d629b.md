---
task_id: d141ec85-292a-4038-98ff-c7ffed7d629b
title: '[منطق] Inconsistent Telegram environment variable naming'
type: logic_audit
priority: medium
...
created_at: '2026-06-06T07:58:48.867052+00:00'
updated_at: '2026-06-06T07:58:48.867054+00:00'
---

# [منطق] Inconsistent Telegram environment variable naming
...
There is an inconsistency in the naming of Telegram bot token environment variables across components. `backend/app/notifications.py` expects `NOTIFY_TELEGRAM_BOT_TOKEN`, while `backend/services/telegram/config.js` expects `TELEGRAM_BOT_TOKEN`. This suggests either two separate Telegram bots are required (which is not explicitly stated and adds complexity) or an inconsistent naming convention for what should be a single shared bot token.

## 💥 پیامد (impact)
Potential for misconfiguration, requiring two separate environment variables for what might be the same Telegram bot, or making it unclear which token should be used by which service. This can lead to deployment errors, confusion during setup, or unexpected behavior where one part of the system fail

## 🛠 پیشنهاد رفع اولیه
Standardize the naming of Telegram-related environment variables. If it's the same bot, use a single, consistent name (e.g., `TELEGRAM_BOT_TOKEN` or `APP_TELEGRAM_BOT_TOKEN`) across all components. If different bots are indeed intended, clarify this in the documentation and ensure distinct, unambiguous naming conventions for each.
...
### Step 1: تصمیم‌گیری و استانداردسازی نام متغیر محیطی Telegram Bot Token
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی ناسازگاری فعلی در نام‌گذاری متغیرهای محیطی Telegram Bot Token و تصمیم‌گیری نهایی برای یک نام استاندارد و یکپارچه در سراسر پروژه است. هدف این است که مشخص شود آیا یک ربات تلگرام واحد مورد نظر است یا چندین ربات، و بر اساس آن، نام متغیر محیطی (مثلاً `TELEGRAM_BOT_TOKEN` یا `APP_TELEGRAM_BOT_TOKEN`) انتخاب شود. این مرحله شامل هیچ تغییر کدی نیست، بلکه یک تصمیم معماری و نام‌گذاری است.
**Excerpt:**
```
Standardize the naming of Telegram-related environment variables. If it's the same bot, use a single, consistent name (e.g., `TELEGRAM_BOT_TOKEN` or `APP_TELEGRAM_BOT_TOKEN`) across all components. If different bots are indeed intended, clarify this in the documentation and ensure distinct, unambiguous naming conventions for each.
```

### Step 2: به‌روزرسانی `backend/app/notifications.py` برای استفاده از نام متغیر محیطی استاندارد
**Status:** `pending` (0%)
**Scope:** این مرحله بر روی اصلاح فایل `backend/app/notifications.py` متمرکز است. کد مربوط به بازیابی Telegram bot token باید به‌روزرسانی شود تا به جای `NOTIFY_TELEGRAM_BOT_TOKEN`، از نام متغیر محیطی استاندارد شده که در مرحله قبل تصمیم‌گیری شد (مثلاً `TELEGRAM_BOT_TOKEN`) استفاده کند. این تغییر فقط در این فایل اعمال می‌شود و هیچ بخش دیگری از سیستم را تحت تأثیر قرار نمی‌دهد.
**Excerpt:**
```
`backend/app/notifications.py` expects `NOTIFY_TELEGRAM_BOT_TOKEN`, while `backend/services/telegram/config.js` expects `TELEGRAM_BOT_TOKEN`. This suggests either two separate Telegram bots are required (which is not explicitly stated and adds complexity) or an inconsistent naming convention for what should be a single shared bot token. Standardize the naming of Telegram-related environment variables. If it's the same bot, use a single, consistent name (e.g., `TELEGRAM_BOT_TOKEN` or `APP_TELEGRAM_BOT_TOKEN`) across all components.
```

### Step 3: تأیید یا به‌روزرسانی `backend/services/telegram/config.js` برای همسویی با نام استاندارد
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی فایل `backend/services/telegram/config.js` است. با توجه به اینکه این فایل در حال حاضر `TELEGRAM_BOT_TOKEN` را انتظار دارد، این مرحله تأیید می‌کند که آیا نام استاندارد شده انتخاب شده در مرحله ۱ با نام فعلی این فایل مطابقت دارد یا خیر. اگر نام استاندارد شده متفاوت باشد (مثلاً `APP_TELEGRAM_BOT_TOKEN`)، این فایل نیز باید به‌روزرسانی شود. در غیر این صورت، فقط تأیید می‌شود که نیازی به تغییر نیست.
**Excerpt:**
```
`backend/services/telegram/config.js` expects `TELEGRAM_BOT_TOKEN`. This suggests either two separate Telegram bots are required (which is not explicitly stated and adds complexity) or an inconsistent naming convention for what should be a single shared bot token. Standardize the naming of Telegram-related environment variables. If it's the same bot, use a single, consistent name (e.g., `TELEGRAM_BOT_TOKEN` or `APP_TELEGRAM_BOT_TOKEN`) across all components.
```

### Step 4: به‌روزرسانی متغیرهای محیطی در محیط‌های استقرار
**Status:** `pending` (0%)
**Scope:** این مرحله شامل به‌روزرسانی متغیرهای محیطی واقعی در تمام محیط‌های استقرار (مانند توسعه، تست، تولید) است. متغیر `NOTIFY_TELEGRAM_BOT_TOKEN` باید حذف شود و متغیر محیطی استاندارد شده جدید (مثلاً `TELEGRAM_BOT_TOKEN`) با مقدار صحیح توکن ربات تلگرام پیکربندی شود. این مرحله حیاتی است تا اطمینان حاصل شود که تغییرات کد در محیط‌های عملیاتی به درستی کار می‌کنند.
**Excerpt:**
```
Potential for misconfiguration, requiring two separate environment variables for what might be the same Telegram bot, or making it unclear which token should be used by which service. This can lead to deployment errors, confusion during setup, or unexpected behavior where one part of the system fail. Standardize the naming of Telegram-related environment variables.
```

### Step 5: به‌روزرسانی مستندات مربوط به متغیر محیطی Telegram Bot Token
**Status:** `pending` (0%)
**Scope:** این مرحله شامل به‌روزرسانی یا ایجاد مستندات پروژه (مانند `README.md` یا فایل‌های `docs/`) است تا نام استاندارد شده متغیر محیطی Telegram Bot Token (مثلاً `TELEGRAM_BOT_TOKEN`) و نحوه پیکربندی آن به وضوح توضیح داده شود. اگر در مرحله ۱ تصمیم بر استفاده از چندین ربات با نام‌های متمایز گرفته شده باشد، این مستندات باید آن را نیز روشن کند. هدف این است که توسعه‌دهندگان جدید و فعلی به راحتی بتوانند سیستم را پیکربندی کنند.
**Excerpt:**
```
If different bots are indeed intended, clarify this in the documentation and ensure distinct, unambiguous naming conventions for each. Potential for misconfiguration, requiring two separate environment variables for what might be the same Telegram bot, or making it unclear which token should be used by which service. This can lead to deployment errors, confusion during setup...
```

### Step 6: پیاده‌سازی یا به‌روزرسانی تست‌های یکپارچه‌سازی برای pipeline نوتیفیکیشن تلگرام
**Status:** `pending` (0%)
**Scope:** این مرحله شامل ایجاد یا به‌روزرسانی تست‌های یکپارچه‌سازی (integration tests) است که به طور خاص تأیید می‌کنند pipeline `notification` به درستی توکن ربات تلگرام را از متغیر محیطی استاندارد شده بازیابی و استفاده می‌کند. این تست‌ها باید فراتر از unit test ها باشند و تعامل بین کامپوننت‌های مختلف را در یک محیط نزدیک به واقعیت بررسی کنند تا از بروز مجدد ناسازگاری‌های پیکربندی جلوگیری شود.
**Excerpt:**
```
This class bug ها در test معمولی پیدا نمی‌شوند چون unit test ها در silo اجرا می‌شوند. coherence issue یعنی دو بخش کد فرض‌های ناسازگار دارند — معمولاً نشانه‌ی refactor ناتمام یا feature flag rot است.
```
