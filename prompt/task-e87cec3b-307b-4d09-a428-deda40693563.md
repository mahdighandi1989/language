---
task_id: e87cec3b-307b-4d09-a428-deda40693563
title: 'فیلد بلااستفاده: Notification.priority'
type: cleanup
priority: low
...
created_at: '2026-06-06T07:59:12.052658+00:00'
updated_at: '2026-06-06T07:59:12.172480+00:00'
target_files:
- backend/app/notifications.py
---

# فیلد بلااستفاده: Notification.priority

## Raw Idea

## 📋 شرح
فیلد `priority` در dataclass/model `Notification` (فایل `backend/app/notifications.py`) تعریف شده ولی هرگز خوانده نمی‌شود.

## 🤔 چرا مهم است
یا (الف) ذخیره می‌شود برای آینده ولی هیچ‌کس مصرف نمی‌کند (waste)، یا (ب) reader gone است (regression — قبلاً مصرف می‌شد).

## 🔍 جزئیات
- علت: field defined in Notification but not read/written anywhere
---
[scan #2 at 2026-06-06T07:59:12.118113+00:00]
## 📋 شرح
فیلد `silent` در dataclass/model `Notification` (فایل `backend/app/notifications.py`) تعریف شده ولی هرگز خوانده نمی‌شود.
...
- علت: field defi
---
[scan #3 at 2026-06-06T07:59:12.145833+00:00]
## 📋 شرح
فیلد `delivered` در dataclass/model `Notification` (فایل `backend/app/notifications.py`) تعریف شده ولی هرگز خوانده نمی‌شود.
...
- علت: field d
---
[scan #4 at 2026-06-06T07:59:12.172474+00:00]
## 📋 شرح
فیلد `created_at` در dataclass/model `Notification` (فایل `backend/app/notifications.py`) تعریف شده ولی هرگز خوانده نمی‌شود.
...
- [ ] تأیید شد `priority` در هیچ reader مصرف نمی‌شود
...
1. گام ۱: grep روی `\.priority` + `['priority']` در کل کدبیس.
...
1. تأیید شد `priority` در هیچ reader مصرف نمی‌شود
2. یا حذف شد (با migration در صورت persist)، یا reader اضافه شد
3. تست‌های schema و serialization عبور می‌کنند
