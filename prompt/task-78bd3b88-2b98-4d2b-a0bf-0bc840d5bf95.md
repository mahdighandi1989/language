---
task_id: 78bd3b88-2b98-4d2b-a0bf-0bc840d5bf95
title: تابع mountFallbacks در controllers/index.js صادر نمی‌شود ولی server.js آن را import می‌کند
type: bug
priority: critical
...
created_at: '2026-06-13T08:07:28.902359+00:00'
updated_at: '2026-06-13T08:15:18.470980+00:00'
target_files:
- backend/controllers/index.js
- backend/server.js
---

# تابع mountFallbacks در controllers/index.js صادر نمی‌شود ولی server.js آن را import می‌کند

## Raw Idea

در backend/server.js خط ۲۷ `import { mountFallbacks } from './controllers/index.js'` انجام می‌شود و در خط ۱۰۸ `mountFallbacks(app)` فراخوانی می‌شود. اما backend/controllers/index.js فقط این فایل‌ها را re-export می‌کند: analysisController, analyticsController, audioController, fallbackController, geminiController, uploadController. اگر `mountFallbacks` یک named export از fallbackController.js باشد، از طریق `export * from './fallbackController.js'` پوشش داده می‌شود — اما این باید صراحتاً تأیید شود. این یک تضاد contract میان barrel و مصرف‌کننده است: server.js صریحاً به وجود نام `mountFallbacks` در سطح barrel وابسته است و اگر fallbackController آن را با نام دیگری یا به‌صورت default صادر کند (که barrel آن را forward نمی‌کند طبق کامنت خط ۸)، در runtime با undefined مواجه می‌شویم و سرور هنگام boot crash می‌کند. همین موضوع برای `applySecurity`/`generalLimiter` (از middleware/index.js) و `attachLiveProxy`/`attachLiveWsObserver`/`attachTelegram` (از services/index.js) صادق است.
...
- `backend/controllers/index.js:11-16` — `barrel exports` — mountFallbacks باید از fallbackController.js به‌صورت named صادر شده باشد
  ```jsx
  export * from './fallbackController.js';
  export * from './geminiController.js';
  ```
- `backend/server.js:27` — `mountFallbacks import` — مصرف‌کننده‌ای که به وجود این نام در barrel وابسته است
  ```jsx
...
Node.js ESM + Express + WebSocket (ws)

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/controllers/fallbackController.js` (سطر 1) — باید mountFallbacks را به‌صورت named export ارائه دهد
- `backend/middleware/index.js` (سطر 10) — applySecurity و generalLimiter باید forward شوند
- `backend/server.js` — این فایل `index.js` را import می‌کند (caller)
...
## 🌐 نقشهٔ وابستگی‌ها
server.js نقطهٔ composition root است و به ۳ barrel (controllers, middleware, services) وابسته است؛ هر symbol گمشده‌ای boot کل سرور را شکست می‌دهد.
...
- [ ] import { mountFallbacks } from './controllers/index.js' مقدار غیر-undefined برمی‌گرداند
- [ ] server.js بدون ReferenceError/TypeError هنگام boot اجرا می‌شود
- [ ] تست smoke که همهٔ named importهای server.js را بررسی می‌کند اضافه شود
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. محتوای backend/controllers/fallbackController.js را بازبینی کن و مطمئن شو `mountFallbacks` به‌صورت named export صادر می‌شود (نه default). همچنین تأیید کن middleware/index.js نام `generalLimiter` و `applySecurity` را forward می‌کند (rateLimiter.js و security.js هر دو به‌صورت named صادر می‌کنند که OK است). یک تست boot ساده اضافه کن که server.js را import کند و مطمئن شود همهٔ symbolهای مصرف‌شده undefined نیستند.

## 💡 نمونه‌های قبل/بعد
**تأیید export در fallbackController**

_قبل:_
```
export default function mountFallbacks(app) { ... }
```
...
## 🧪 دستورات اعتبارسنجی
- `node -e "import('./backend/controllers/index.js').then(m => console.assert(typeof m.mountFallbacks==='function'))"`
- `python -m pytest tests/backend/test_server.py`

## ⚠️ ریسک‌ها و موارد احتیاط
اگر fallbackController از default export استفاده کند، نیاز به تبدیل به named و به‌روزرسانی هر مصرف‌کنندهٔ دیگر
...
1. import { mountFallbacks } from './controllers/index.js' مقدار غیر-undefined برمی‌گرداند _(verify: backend_test)_
2. server.js بدون ReferenceError/TypeError هنگام boot اجرا می‌شود _(verify: backend_test)_
3. تست smoke که همهٔ named importهای server.js را بررسی می‌کند اضافه شود _(verify: backend_test)_
