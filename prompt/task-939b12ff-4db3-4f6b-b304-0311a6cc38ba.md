---
task_id: 939b12ff-4db3-4f6b-b304-0311a6cc38ba
title: '[منطق] Telegram Service Mutation Paths Lack Explicit Authorization'
type: logic_audit
priority: critical
...
created_at: '2026-06-06T07:55:56.618794+00:00'
updated_at: '2026-06-06T07:55:56.618799+00:00'
---

# [منطق] Telegram Service Mutation Paths Lack Explicit Authorization
...
The `backend/services/telegram/practice.js` component orchestrates AI teacher practice, which involves significant mutation paths such as managing user sessions, storing interaction history in `TelegramStore`, and potentially triggering AI resource usage. The component's `expected_inputs` only list `chatId` (a Telegram-specific identifier) and no explicit authentication token or backend user ID. There is no mention of an authorization mechanism that ensures these mutation operations are performe

## 💥 پیامد (impact)
Unauthorized users could potentially initiate or manipulate sessions, consume AI resources, or modify data in `TelegramStore` without proper authorization. This can lead to service abuse, unexpected costs, data integrity issues, and prevents the implementation of granular access controls like user-s

## 🛠 پیشنهاد رفع اولیه
Implement a robust authorization layer for the Telegram service. This should involve linking Telegram `chatId` to a unique backend `userId` (e.g., Firebase UID) and passing this `userId` as an `expected_input` to `practice.js`. Before any mutation operation, perform explicit permission checks using the `userId` against a defined authorization policy.
