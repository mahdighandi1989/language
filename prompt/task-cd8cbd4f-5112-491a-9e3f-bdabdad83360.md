---
task_id: cd8cbd4f-5112-491a-9e3f-bdabdad83360
title: نبود timeout روی همهٔ fetchهای Gemini باعث hang نامحدود درخواست‌ها می‌شود
type: bug
priority: high
...
# نبود timeout روی همهٔ fetchهای Gemini باعث hang نامحدود درخواست‌ها می‌شود

## Raw Idea

در backend/services/geminiService.js همهٔ فراخوانی‌های fetch (analyzeWithGemini، uploadToGeminiFileAPI، analyzeWithGeminiFileAPIWithModel، deleteGeminiFile، polling وضعیت فایل) هیچ AbortController/timeout ندارند. اگر سرویس Gemini کند یا غیرپاسخگو باشد، درخواست HTTP کلاینت (مثل /api/gemini/chat یا /api/analyze-files) به‌طور نامحدود معلق می‌ماند و منابع سرور (socket، رشتهٔ event loop در حال انتظار) را اشغال می‌کند. این یک failure mode کلاسیک «API call بدون timeout» است که در critical path قرار دارد چون تمام تحلیل فایل و چت از این توابع عبور می‌کنند.
...
- `backend/services/geminiService.js:15-24` — `analyzeWithGemini` — fetch بدون signal/timeout
  ```jsx
  ...
  ```
- `backend/services/geminiService.js:106-120` — `uploadToGeminiFileAPI (polling loop)` — polling طولانی + fetch بدون timeout
  ```jsx
...
Node 18+ native fetch (AbortController پشتیبانی می‌شود)

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/services/analysisService.js` (سطر 82) — تمام مسیرهای تحلیل از این توابع gemini استفاده می‌کنند
- `backend/controllers/geminiController.js` — chat/tts را از طریق همین کلاینت صدا می‌زند
- `backend/config/env.js` — `geminiService.js` این فایل را import می‌کند

## 🌐 نقشهٔ وابستگی‌ها
geminiService در analysisService و geminiController مصرف می‌شود؛ افزودن timeout روی هر دو مسیر چت و تحلیل فایل اثر مثبت دارد و رفتار موفق را تغییر نمی‌دهد.
...
- [ ] اگر Gemini ظرف timeout پاسخ ندهد، تابع یک خطای مشخص (مثلاً 'Gemini request timed out') throw می‌کند
- [ ] کنترلر چت برای timeout کد 504 یا 502 برمی‌گرداند نه hang نامحدود
- [ ] تست واحد با fetch ساختگی کند، رفتار timeout را تأیید می‌کند
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. یک helper مشترک fetchWithTimeout(url, opts, ms) با AbortController بساز و timeout معقول (مثلاً 60s برای generateContent و 300s برای آپلود فایل بزرگ) تنظیم کن. روی timeout یک خطای واضح throw کن تا کنترلر بتواند 504/502 برگرداند.
...
_بعد:_
```
const response = await fetchWithTimeout(apiUrl, { method: 'POST', headers, body }, 60000);
```
...
- `python -m pytest tests/integration/test_ai_llm_pipeline.py`

## ⚠️ ریسک‌ها و موارد احتیاط
تنظیم timeout بیش از حد کوتاه ممکن است تحلیل فایل‌های بزرگ مشروع را قطع کند؛ مقادیر باید جدا برای آپلود و generateContent تنظیم شوند.
