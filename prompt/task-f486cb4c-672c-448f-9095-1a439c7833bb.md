---
task_id: f486cb4c-672c-448f-9095-1a439c7833bb
title: کد وضعیت HTTP نامناسب برای عدم پیکربندی کلید Gemini API
type: bug
priority: medium
...
created_at: '2026-06-13T08:12:02.494687+00:00'
updated_at: '2026-06-13T08:18:37.321416+00:00'
target_files:
- backend/middleware/requireGeminiKey.js
---

# کد وضعیت HTTP نامناسب برای عدم پیکربندی کلید Gemini API

## Raw Idea

در `backend/middleware/requireGeminiKey.js`، اگر `GEMINI_API_KEY` پیکربندی نشده باشد، middleware با کد وضعیت `500 Internal Server Error` پاسخ می‌دهد. این کد وضعیت معمولاً برای خطاهای غیرمنتظره سرور استفاده می‌شود، در حالی که عدم پیکربندی یک کلید API یک مشکل عملیاتی یا پیکربندی است که بهتر است با `503 Service Unavailable` (سرویس در دسترس نیست) یا `403 Forbidden` (اگر به معنای عدم دسترسی به قابلیت باشد) نشان داده شود. `503` دقیق‌تر است زیرا نشان می‌دهد قابلیت به دلیل پیکربندی سرور در دسترس نیست.
...
- `backend/middleware/requireGeminiKey.js:5-7` — `requireGeminiKey` — خطی که کد وضعیت 500 را برمی‌گرداند.
  ```jsx
...
- `backend/routes/index.js` (سطر 37) — این middleware در چندین مسیر Gemini استفاده می‌شود.
- `backend/config/env.js` (سطر 1) — منبع `GEMINI_API_KEY`.

## 🌐 نقشهٔ وابستگی‌ها
این middleware توسط تمام مسیرهای API که به Gemini نیاز دارند استفاده می‌شود. تغییر کد وضعیت بر نحوهٔ تفسیر خطاهای پیکربندی توسط کلاینت‌ها تأثیر می‌گذارد.
...
- [ ] در صورت عدم پیکربندی `GEMINI_API_KEY`، درخواست به مسیرهای Gemini با کد وضعیت 503 پاسخ دهد.
- [ ] پیام خطا همچنان `API key not configured` باشد.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. کد وضعیت HTTP را در `backend/middleware/requireGeminiKey.js` از `500` به `503` تغییر دهید تا وضعیت عدم پیکربندی API Key را بهتر منعکس کند.
...
## 🧪 دستورات اعتبارسنجی
- `تست واحد جدیدی برای `requireGeminiKey` اضافه شود که عدم پیکربندی کلید را شبیه‌سازی کند و کد وضعیت 503 را بررسی کند.`
- `تست E2E برای مسیرهای Gemini که بدون `GEMINI_API_KEY` اجرا می‌شوند، کد وضعیت 503 را تأیید کند.`

## ⚠️ ریسک‌ها و موارد احتیاط
پایین؛ فقط کد وضعیت خطا را تغییر می‌دهد.
...
1. در صورت عدم پیکربندی `GEMINI_API_KEY`، درخواست به مسیرهای Gemini با کد وضعیت 503 پاسخ دهد. _(verify: api_response)_
2. پیام خطا همچنان `API key not configured` باشد. _(verify: api_response)_
