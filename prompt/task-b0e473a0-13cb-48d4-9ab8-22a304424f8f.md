---
task_id: b0e473a0-13cb-48d4-9ab8-22a304424f8f
title: Inconsistent Firebase Configuration Loading and Usage
type: refactor
priority: medium
...
created_at: '2026-06-13T08:12:02.537589+00:00'
updated_at: '2026-06-13T08:18:47.090099+00:00'
target_files:
- backend/middleware/firebaseAuth.js
- backend/server.js
---

# Inconsistent Firebase Configuration Loading and Usage

## Raw Idea

The Firebase Admin SDK is initialized in `backend/middleware/firebaseAuth.js` by directly reading `process.env.FIREBASE_PROJECT_ID`. Separately, `backend/server.js` uses `firebaseConfig.projectId` (presumably from `backend/config/bootstrap.js`) to check Firebase configuration status for the Telegram bot. This creates two distinct paths for Firebase configuration, increasing the risk of inconsistencies or race conditions if the environment variables or initialization logic diverge. A single, centralized source for Firebase configuration and initialization should be used.
...
- `backend/middleware/firebaseAuth.js:10-31` — `ensureAdmin` — The `ensureAdmin` function directly accesses `process.env`.
  ```jsx
  ...
  ```
- `backend/server.js:91-91` — Uses `firebaseConfig.projectId` from `bootstrap.js`.
  ```jsx
...
- `backend/config/bootstrap.js` — Likely the primary Firebase config source, should be consumed by `firebaseAuth.js`.
- `backend/services/telegram/index.js` — Imports `requireAuth` from `firebaseAuth.js` and uses `firebaseConfig.projectId`.
- `backend/config/env.js` — Defines `FIREBASE_PROJECT_ID` which is read by `firebaseAuth.js`.
- `backend/routes/index.js` — این فایل `firebaseAuth.js` را import می‌کند (caller)
...
## 🌐 نقشهٔ وابستگی‌ها
`firebaseAuth.js` is a core middleware used by multiple routes and the Telegram service. Changes here will affect all authenticated endpoints.
...
- [ ] `backend/middleware/firebaseAuth.js` no longer directly accesses `process.env.FIREBASE_PROJECT_ID`.
- [ ] Firebase Admin SDK is initialized once and passed or accessed consistently across the backend.
- [ ] All Firebase-dependent services (auth, Telegram) function correctly.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Centralize Firebase configuration loading. `firebaseAuth.js` should receive the initialized Firebase Admin app or its configuration from a single source (e.g., `backend/config/bootstrap.js` or `backend/server.js`) rather than re-reading environment variables. The `ensureAdmin` function should be refactored to accept the Firebase Admin app instance.

## 💡 نمونه‌های قبل/بعد
**Refactor `ensureAdmin` to accept config**
...
_بعد:_
```
function ensureAdmin(projectId) { /* ... */ admin.initializeApp({ projectId }); }
```
...
## 🧪 دستورات اعتبارسنجی
- `npm run test -- backend/tests/test_firebase_auth.py`
- `npm run test -- backend/tests/test_telegram_service.js`

## ⚠️ ریسک‌ها و موارد احتیاط
Potential to break authentication or Firebase-dependent features if not carefully refactored.
...
1. `backend/middleware/firebaseAuth.js` no longer directly accesses `process.env.FIREBASE_PROJECT_ID`. _(verify: static)_
2. Firebase Admin SDK is initialized once and passed or accessed consistently across the backend. _(verify: manual_only)_
3. All Firebase-dependent services (auth, Telegram) function correctly. _(verify: backend_test)_
