---
task_id: 9293a408-a1bf-46af-8aaf-31ec448a87e3
...
created_at: '2026-06-06T07:57:45.639022+00:00'
updated_at: '2026-06-06T08:05:37.149601+00:00'
target_files:
...
The `env.js` module, whose primary purpose is to load and export environment variables, implicitly relies on `validateEnv` to perform complex decryption (AES-256-GCM) in addition to validation. While `validateEnv` is described as handling this 'transparently', this design couples the fundamental configuration loading process with a specific secret decryption mechanism. This adds an unnecessary lay

📁 file: backend/config/env.js (line 12)
...
- `backend/config/validateEnv.js` — `env.js` این فایل را import می‌کند
- `backend/server.js` — این فایل `env.js` را import می‌کند (caller)
- `backend/middleware/requireGeminiKey.js` — این فایل `env.js` را import می‌کند (caller)
- `backend/services/geminiService.js` — این فایل `env.js` را import می‌کند (caller)
- `backend/services/liveProxyService.js` — این فایل `env.js` را import می‌کند (caller)
