---
task_id: 701af2f4-75d0-45f1-84de-4565d51ebc48
title: دو نسخه از App.jsx (frontend/src/App.jsx و frontend/src/components/App.jsx) وجود دارد — duplicate/stale component
type: refactor
priority: high
...
created_at: '2026-06-13T08:10:56.533667+00:00'
updated_at: '2026-06-13T08:17:25.273718+00:00'
target_files:
- frontend/src/App.jsx
- frontend/src/components/App.jsx
---

# دو نسخه از App.jsx (frontend/src/App.jsx و frontend/src/components/App.jsx) وجود دارد — duplicate/stale component

## Raw Idea

در ساختار پروژه هم frontend/src/App.jsx و هم frontend/src/components/App.jsx وجود دارند، هر دو با تست همراه (App.test.jsx و components/App.test.jsx). طبق نقشهٔ Importهای داخلی، `frontend/src/components/App.jsx` توسط main.jsx و دو تست import می‌شود — یعنی نسخهٔ فعال همان components/App.jsx است. این یعنی frontend/src/App.jsx احتمالاً stale است یا فقط re-export می‌کند. وجود دو فایل با نام یکسان در دو مسیر، سردرگمی ایجاد می‌کند و ریسک ویرایش فایل اشتباه را بالا می‌برد. README هم به `frontend/src/App.jsx` به‌عنوان «root component» اشاره می‌کند که با واقعیت import (components/App.jsx) ناسازگار است (contract/docs mismatch).
...
- `frontend/src/App.jsx:1-1` — `App component (نسخهٔ مشکوک stale)` — نسخه‌ای که main.jsx آن را import نمی‌کند
  ```jsx
  // frontend/src/App.jsx — احتمالاً نسخهٔ قدیمی یا re-export
  ```
- `frontend/src/components/App.jsx:1-12` — `App (نسخهٔ فعال)` — نسخهٔ فعال که توسط main.jsx import می‌شود
  ```jsx
  // Purpose: root React component for the Lebanese-dialect learning app.
  ```
...
- `frontend/src/main.jsx` — components/App.jsx را import می‌کند نه src/App.jsx
- `README.md` — به frontend/src/App.jsx به‌عنوان root اشاره می‌کند که با واقعیت ناسازگار است
- `frontend/src/App.test.jsx` — تست نسخهٔ مشکوک
...
## 🌐 نقشهٔ وابستگی‌ها
components/App.jsx هاب اصلی فرانت‌اند است (۳ مصرف‌کننده طبق نقشهٔ import)؛ src/App.jsx خارج از این گراف است و احتمالاً orphan/stale.
...
- [ ] فقط یک پیاده‌سازی واقعی App وجود دارد؛ دیگری حذف یا re-export است
- [ ] main.jsx و README به مسیر واحد ارجاع می‌دهند
- [ ] تست‌های frontend بدون شکست اجرا می‌شوند
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. بررسی کن frontend/src/App.jsx چه می‌کند: اگر فقط `export { default } from './components/App.jsx'` است، آن را نگه‌دار ولی در README مسیر صحیح را اصلاح کن؛ اگر کد قدیمی/تکراری دارد، آن را حذف کن و همهٔ ارجاعات (main.jsx) را به یک مسیر واحد یکپارچه کن. تست‌های تکراری (App.test.jsx vs components/App.test.jsx) را نیز ادغام کن.

## 💡 نمونه‌های قبل/بعد
**یکپارچه‌سازی re-export**

_قبل:_
```
// src/App.jsx یک کپی کامل از کامپوننت
```
...
- `grep -rn "from './App'" frontend/src/`

## ⚠️ ریسک‌ها و موارد احتیاط
حذف اشتباه نسخهٔ فعال؛ نیاز به اطمینان از اینکه import در main.jsx قطع نشود
...
1. فقط یک پیاده‌سازی واقعی App وجود دارد؛ دیگری حذف یا re-export است _(verify: static)_
2. main.jsx و README به مسیر واحد ارجاع می‌دهند _(verify: static)_
3. تست‌های frontend بدون شکست اجرا می‌شوند _(verify: backend_test)_
