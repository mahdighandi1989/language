---
task_id: 1f60c988-6373-4955-9ffc-8f80e398c271
title: duplicate منطق buildAllowedOrigins بین backend/server.js و backend/middleware/security.js
type: refactor
priority: medium
...
created_at: '2026-06-06T07:57:19.322135+00:00'
updated_at: '2026-06-06T07:57:19.322138+00:00'
target_files:
- backend/server.js
---

# duplicate منطق buildAllowedOrigins بین backend/server.js و backend/middleware/security.js

## Raw Idea

منطق ساخت allow-list مبدأهای CORS (ترکیب CORS_ORIGIN, FRONTEND_URL و DEV_ORIGIN با split/trim/filter/dedup) دقیقاً در دو جا کپی شده: backend/server.js خطوط 46-54 (به‌صورت inline) و backend/middleware/security.js خطوط 15-29 (تابع export شدهٔ buildAllowedOrigins). کامنت در security.js (خطوط 11-13) صراحتاً می‌گوید این تابع «single source of truth» برای منطق allow-list است، ولی server.js از آن استفاده نمی‌کند و نسخهٔ inline خودش را دارد. این یک تناقض contract آشکار است: تابعی که به‌عنوان منبع واحد حقیقت اعلام شده، توسط مصرف‌کنندهٔ اصلی‌اش (server.js) دور زده می‌شود. هر تغییر در منطق (مثلاً افزودن یک مبدأ مجاز جدید) باید در دو جا اعمال شود.
...
- `backend/server.js:46-54` — `allowedOrigins (inline)`
  ```jsx
...
1. در backend/server.js به‌جای منطق inline خطوط 46-54، تابع buildAllowedOrigins را از './middleware/security.js' import کن و const allowedOrigins = buildAllowedOrigins(); بگذار. این کار duplication را حذف می‌کند و ادعای «single source of truth» در security.js را واقعی می‌سازد.
