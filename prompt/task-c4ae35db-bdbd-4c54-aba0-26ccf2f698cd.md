---
task_id: c4ae35db-bdbd-4c54-aba0-26ccf2f698cd
title: نادیده گرفتن توکن‌های احراز هویت نامعتبر در `optionalAuth` هنگام عدم پیکربندی Firebase Admin
type: bug
priority: high
...
# نادیده گرفتن توکن‌های احراز هویت نامعتبر در `optionalAuth` هنگام عدم پیکربندی Firebase Admin

## Raw Idea

در `backend/middleware/firebaseAuth.js`، middleware `optionalAuth` اگر `ensureAdmin()` (که Firebase Admin SDK را مقداردهی اولیه می‌کند) `false` برگرداند و یک توکن احراز هویت در درخواست وجود داشته باشد، به جای رد کردن درخواست با `401 Unauthorized`، به سادگی `next()` را فراخوانی می‌کند. این رفتار باعث می‌شود توکن‌های نامعتبر (که به دلیل عدم پیکربندی سرویس Firebase Admin قابل تأیید نیستند) نادیده گرفته شوند و درخواست به لایه‌های بعدی منتقل شود، که می‌تواند منجر به تصمیمات نادرست در مورد دسترسی کاربر شود. این با هدف 'soft auth guard' که می‌گوید توکن‌های ارائه شده باید تأیید شوند، در تضاد است.
...
- `backend/middleware/firebaseAuth.js:69-72` — `optionalAuth` — این بلوک باید به جای `next()`، یک خطای 401 برگرداند.
  ```jsx
  if (!ensureAdmin()) {
...
Node.js, Express.js middleware, Firebase Admin SDK

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/routes/index.js` (سطر 37) — این middleware در چندین مسیر API استفاده می‌شود.
- `backend/services/telegram/index.js` (سطر 25) — این middleware در مسیر `link` تلگرام استفاده می‌شود.

## 🌐 نقشهٔ وابستگی‌ها
این middleware توسط مسیرهای API که نیاز به احراز هویت اختیاری دارند استفاده می‌شود. اصلاح آن، رفتار احراز هویت را برای کاربران با توکن‌های نامعتبر در محیط‌های بدون پیکربندی Firebase Admin تغییر می‌دهد.
...
- [ ] هنگامی که `FIREBASE_PROJECT_ID` تنظیم نشده است و یک توکن احراز هویت نامعتبر به یک مسیر `optionalAuth` ارسال می‌شود، پاسخ 401 Unauthorized باشد.
- [ ] هنگامی که `FIREBASE_PROJECT_ID` تنظیم نشده است و هیچ توکن احراز هویتی ارسال نمی‌شود، درخواست همچنان به لایه‌های بعدی منتقل شود.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. در `optionalAuth`، اگر توکنی ارائه شده و `ensureAdmin()` `false` برگرداند، باید با `res.status(401).json({ error: 'سرویس احراز هویت پیکربندی نشده است' });` پاسخ دهد، همانند `requireAuth`.
...
## 🧪 دستورات اعتبارسنجی
- `تست واحد جدیدی برای `optionalAuth` اضافه شود که سناریوی عدم پیکربندی Firebase Admin و ارسال توکن را پوشش دهد.`
- `تست E2E برای مسیرهای `optionalAuth` که بدون `FIREBASE_PROJECT_ID` و با توکن نامعتبر اجرا می‌شوند، کد وضعیت 401 را تأیید کند.`

## ⚠️ ریسک‌ها و موارد احتیاط
پایین؛ رفتار احراز هویت را در یک سناریوی خاص (عدم پیکربندی Firebase Admin) اصلاح می‌کند.
