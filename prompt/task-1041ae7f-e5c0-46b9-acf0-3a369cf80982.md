---
task_id: 1041ae7f-e5c0-46b9-acf0-3a369cf80982
title: Gemini API key passed as URL query parameter in every request, exposing it in logs/proxies
type: security
priority: high
...
created_at: '2026-06-13T08:10:28.337192+00:00'
updated_at: '2026-06-13T08:16:49.690308+00:00'
target_files:
...
# Gemini API key passed as URL query parameter in every request, exposing it in logs/proxies

## Raw Idea

همه‌ی فراخوانی‌های Gemini در geminiService.js کلید API را به‌صورت query parameter `?key=${GEMINI_API_KEY}` در URL قرار می‌دهند. URLها معمولاً در logهای سرور، access logهای reverse proxy، APM/tracing tools و مرورگرها ذخیره می‌شوند، بنابراین کلید حساس Gemini در مسیرهای متعدد نشت می‌کند. روش امن، ارسال کلید در header `x-goog-api-key` است. این الگو در ۵ نقطه (analyzeWithGemini، uploadToGeminiFileAPI با چند fetch، polling، analyzeWithGeminiFileAPIWithModel، deleteGeminiFile) تکرار شده است.
...
- `backend/services/geminiService.js:7-19` — `analyzeWithGemini` — کلید در query string؛ باید به header منتقل شود
  ```jsx
  ...
  ```
- `backend/services/geminiService.js:39-53` — `uploadToGeminiFileAPI` — start upload و سایر fetchها هم همین مشکل را دارند (خطوط 110-112, 169-171)
  ```jsx
  const startUploadUrl = `${GEMINI_BASE}/upload/v1beta/files?key=${GEMINI_API_KEY}`;
  ```
- `backend/services/geminiService.js:131` — `analyzeWithGeminiFileAPIWithModel` — همین الگوی نشت کلید
  ```jsx
...
Node 18+ Express, native fetch, Google Generative Language REST API

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/config/env.js` (سطر 1) — منبع GEMINI_API_KEY که decrypt و export می‌شود
- `backend/services/analysisService.js` (سطر 4) — همه‌ی توابع geminiService را فراخوانی می‌کند
- `backend/utils/redact.js` — redactSensitiveData باید کلید را در پیام خطاهای fetch پنهان کند

## 🌐 نقشهٔ وابستگی‌ها
geminiService توسط analysisService و geminiController استفاده می‌شود؛ تغییر به header-based auth رفتاری شفاف است و فقط نحوه‌ی ساخت request را عوض می‌کند، خروجی API تغییر نمی‌کند.
...
- [ ] هیچ URL در backend/services/geminiService.js شامل `key=` نیست (grep بدون نتیجه)
- [ ] همه‌ی fetchها header `x-goog-api-key` دارند
- [ ] تست موجود geminiController/analysis همچنان pass می‌شود
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. کلید را از query string حذف کرده و در همه‌ی fetchها به‌صورت header `'x-goog-api-key': GEMINI_API_KEY` ارسال کن. یک helper مشترک (مثلاً `geminiFetch(url, opts)`) بساز که header را اضافه می‌کند تا تکرار حذف شود و BASE URLها بدون `key=` ساخته شوند.
...
```
const apiUrl = `${GEMINI_BASE}/v1beta/models/gemini-2.5-flash:generateContent?key=${GEMINI_API_KEY}`;
await fetch(apiUrl, { method:'POST', headers:{ 'Content-Type':'application/json' }, body });
```

_بعد:_
```
const apiUrl = `${GEMINI_BASE}/v1beta/models/gemini-2.5-flash:generateContent`;
await fetch(apiUrl, { method:'POST', headers:{ 'Content-Type':'application/json', 'x-goog-api-key': GEMINI_API_KEY }, body });
```
...
## 🧪 دستورات اعتبارسنجی
- `grep -n 'key=\${GEMINI_API_KEY}' backend/services/geminiService.js`
- `npm run test --workspace backend`

## ⚠️ ریسک‌ها و موارد احتیاط
اگر تستی mock URL را با key match کند باید به‌روزرسانی شود
...
1. هیچ URL در backend/services/geminiService.js شامل `key=` نیست (grep بدون نتیجه) _(verify: static)_
2. همه‌ی fetchها header `x-goog-api-key` دارند _(verify: static)_
3. تست موجود geminiController/analysis همچنان pass می‌شود _(verify: backend_test)_
