---
task_id: 95cfdc45-47e8-4712-8b3c-98df2e103e75
title: 'env var بلااستفاده: PORT'
type: cleanup
priority: medium
...
created_at: '2026-06-06T07:57:45.156485+00:00'
updated_at: '2026-06-06T08:05:06.753841+00:00'
---

# env var بلااستفاده: PORT
...
---
[scan #2 at 2026-06-06T07:57:45.190914+00:00]
## 📋 شرح
env var `TELEGRAM_ADMIN_IDS` در `.env`/config تعریف شده ولی در هیچ `os.getenv` یا `process.env` خوانده نمی‌شود.
...
---
[scan #3 at 2026-06-06T07:57:45.209161+00:00]
## 📋 شرح
env var `TELEGRAM_ALLOWED_USER_IDS` در `.env`/config تعریف شده ولی در هیچ `os.getenv` یا `process.env` خوانده نمی‌شود.
...
- علت: documented in
---
[scan #4 at 2026-06-06T07:57:45.230134+00:00]
## 📋 شرح
env var `TELEGRAM_BOT_TOKEN` در `.env`/config تعریف شده ولی در هیچ `os.getenv` یا `process.env` خوانده نمی‌شود.
...
- علت: documented in .env.e
---
[scan #5 at 2026-06-06T07:57:45.253978+00:00]
## 📋 شرح
env var `TELEGRAM_CHAT_ID` در `.env`/config تعریف شده ولی در هیچ `os.getenv` یا `process.env` خوانده نمی‌شود.

## 🤔 چرا مهم است
یا (الف) reader حذف شده و باعث config drift می‌شود، یا (ب) leak اطلاعات حساس به repository است (مخصوصاً اگر secret است).

## 🔍 جزئیات
- علت: documented in .env.exa
---
[scan #6 at 2026-06-06T07:57:45.271334+00:00]
## 📋 شرح
env var `TELEGRAM_DATA_DIR` در `.env`/config تعریف شده ولی در هیچ `os.getenv` یا `process.env` خوانده نمی‌شود.
...
- [ ] `PORT` در هیچ کدی خوانده نمی‌شود (تأیید شده)
...
1. گام ۱: grep روی `PORT` در همه کدبیس + CI configs + Dockerfile.
...
1. `PORT` در هیچ کدی خوانده نمی‌شود (تأیید شده) _(verify: static)_
