...
updated_at: '2026-06-13T08:16:41.240750+00:00'
target_files:
...
1. `frontend/src/components/App.jsx` no longer contains hardcoded `defaultPrompts`. _(verify: static)_
2. A new backend endpoint (`/api/prompts`) serves the canonical prompts from `backend/models/prompts.js`. _(verify: api_response)_
3. The frontend successfully fetches and uses these prompts. _(verify: ui_interaction)_
4. Custom prompts functionality in the frontend still works by overriding the fetched defaults. _(verify: ui_interaction)_
