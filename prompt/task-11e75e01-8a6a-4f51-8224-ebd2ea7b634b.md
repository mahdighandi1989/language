---
task_id: 11e75e01-8a6a-4f51-8224-ebd2ea7b634b
title: Potential Data Inconsistency with localStorage Fallback for User Data
type: bug
priority: high
...
created_at: '2026-06-13T08:09:59.542378+00:00'
updated_at: '2026-06-13T08:16:32.327689+00:00'
target_files:
- frontend/src/components/App.jsx
- README.md
---

# Potential Data Inconsistency with localStorage Fallback for User Data

## Raw Idea

The project uses `localStorage` as a fallback for user data when Firebase is not configured, as stated in `README.md` and implied by `frontend/src/components/App.jsx`'s `initialData` and Firebase usage. There is no clear strategy for synchronizing or migrating user data between `localStorage` and Firestore once Firebase becomes available or unavailable. This can lead to data loss or inconsistent user experiences if a user's Firebase configuration changes or if they switch between environments (e.g., local dev with/without Firebase, then production).
...
- `frontend/src/components/App.jsx:11-12` — `App` — The `App` component manages top-level UI and client state, including data loading/saving.
  ```jsx
  import React, { useState, useEffect, useRef, useMemo, useCallback } from 'react';
  ```
- `README.md:4-5` — Explicitly mentions `localStorage` fallback.
  ```markdown
  User data is
...
React, Firebase Auth/Firestore, Web Storage API.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `frontend/src/firebaseConfig.js` — Determines if Firebase is configured, influencing the fallback.
- `frontend/src/hooks/useFirebase.js` — Handles Firebase authentication state, which impacts data loading.
...
## 🌐 نقشهٔ وابستگی‌ها
This affects all user-specific data and the overall user experience regarding data persistence.
...
- [ ] When Firebase becomes available for a user with existing `localStorage` data, the app prompts for data migration/merge.
- [ ] Data is consistently loaded from either `localStorage` or Firestore, with a clear priority.
- [ ] No data loss occurs when switching between Firebase-configured and non-configured states.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Implement a clear data migration/synchronization strategy between `localStorage` and Firestore. Upon successful Firebase authentication/configuration, check for existing data in `localStorage` and offer to migrate it to Firestore, or merge it. Ensure a consistent data source is used once a decision is made (e.g., always prefer Firestore if available).

## 💡 نمونه‌های قبل/بعد
**Example of localStorage to Firestore migration logic**

_قبل:_
```
useEffect(() => { /* load from Firestore or localStorage */ }, [user]);
```
...
useEffect(() => {
  if (user && hasFirebaseConfig()) {
    // Check localStorage for data, prompt user to migrate/merge to Firestore
...
Complex to implement robustly; potential for data loss if migration logic is flawed.
...
1. When Firebase becomes available for a user with existing `localStorage` data, the app prompts for data migration/merge. _(verify: ui_interaction)_
2. Data is consistently loaded from either `localStorage` or Firestore, with a clear priority. _(verify: ui_interaction)_
3. No data loss occurs when switching between Firebase-configured and non-configured states. _(verify: ui_interaction)_
