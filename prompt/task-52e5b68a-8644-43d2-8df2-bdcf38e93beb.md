---
task_id: 52e5b68a-8644-43d2-8df2-bdcf38e93beb
title: استفاده از مقدار hardcoded برای حداکثر حجم فایل در پیام خطای Multer
type: refactor
priority: low
...
created_at: '2026-06-13T08:13:37.635474+00:00'
updated_at: '2026-06-13T08:20:22.362334+00:00'
target_files:
- backend/middleware/upload.js
---

# استفاده از مقدار hardcoded برای حداکثر حجم فایل در پیام خطای Multer

## Raw Idea

در `backend/middleware/upload.js`، تابع `handleMulterError` برای خطای `LIMIT_FILE_SIZE`، حداکثر حجم فایل را به صورت hardcode شده '500MB' در پیام خطا (`حجم فایل بیش از حد مجاز است (حداکثر 500MB)`) نمایش می‌دهد. این در حالی است که ثابت `MAX_FILE_SIZE` در همان فایل تعریف شده است. اگر `MAX_FILE_SIZE` در آینده تغییر کند، این پیام خطا به روز نخواهد شد و اطلاعات نادرستی به کاربر ارائه می‌دهد. این با نحوهٔ استفاده از `MAX_FILES` برای خطای `LIMIT_FILE_COUNT` که از ثابت استفاده می‌کند، ناسازگار است.
...
- `backend/middleware/upload.js:51-53` — `handleMulterError` — خطی که پیام خطای hardcode شده را برمی‌گرداند.
  ```jsx
...
Node.js, Express.js middleware, Multer

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/middleware/upload.js` (سطر 31) — تعریف ثابت `MAX_FILE_SIZE` در همین فایل.
...
## 🌐 نقشهٔ وابستگی‌ها
این تغییر فقط بر پیام خطای بازگردانده شده به کلاینت در صورت آپلود فایل بزرگتر از حد مجاز تأثیر می‌گذارد و هیچ وابستگی خارجی ندارد.
...
- [ ] هنگام آپلود فایلی بزرگتر از `MAX_FILE_SIZE`، پیام خطا حجم فایل را به صورت پویا و صحیح نمایش دهد.
- [ ] تغییر `MAX_FILE_SIZE` باعث به‌روزرسانی خودکار پیام خطا شود.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. پیام خطای `LIMIT_FILE_SIZE` را به گونه‌ای اصلاح کنید که مقدار `MAX_FILE_SIZE` را به صورت پویا از ثابت تعریف شده بخواند و نمایش دهد. این کار باعث می‌شود پیام خطا همیشه دقیق باشد.

## 💡 نمونه‌های قبل/بعد
**استفاده از ثابت `MAX_FILE_SIZE`**
...
_بعد:_
```
return res.status(400).json({ error: `حجم فایل بیش از حد مجاز است (حداکثر ${MAX_FILE_SIZE / (1024 * 1024)}MB)` });
```
...
## 🧪 دستورات اعتبارسنجی
- `تست واحد جدیدی برای `handleMulterError` اضافه شود که خطای `LIMIT_FILE_SIZE` را شبیه‌سازی کند و محتوای پیام خطا را بررسی کند.`

## ⚠️ ریسک‌ها و موارد احتیاط
پایین؛ فقط یک رشته پیام خطا را تغییر می‌دهد.
...
1. هنگام آپلود فایلی بزرگتر از `MAX_FILE_SIZE`، پیام خطا حجم فایل را به صورت پویا و صحیح نمایش دهد. _(verify: api_response)_
2. تغییر `MAX_FILE_SIZE` باعث به‌روزرسانی خودکار پیام خطا شود. _(verify: static)_
