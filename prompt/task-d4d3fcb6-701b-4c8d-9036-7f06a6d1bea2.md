---
task_id: d4d3fcb6-701b-4c8d-9036-7f06a6d1bea2
title: آسیب‌پذیری احتمالی جعل نوع MIME در آپلود فایل
type: security
priority: high
...
created_at: '2026-06-06T07:55:56.629645+00:00'
updated_at: '2026-06-06T08:01:39.518395+00:00'
target_files:
- backend/middleware/upload.js
---

# آسیب‌پذیری احتمالی جعل نوع MIME در آپلود فایل

## Raw Idea

در `backend/middleware/upload.js`، تابع `isAllowedMimeType` و `fileFilter` Multer برای اعتبارسنجی نوع فایل، صرفاً به `file.mimetype` که توسط کلاینت ارائه می‌شود، تکیه می‌کنند. یک مهاجم می‌تواند با آپلود یک فایل مخرب (مانند یک فایل اجرایی) و جعل نوع MIME آن به یک نوع مجاز (مانند `application/pdf`)، این کنترل را دور بزند. این فایل مخرب سپس در `uploadDir` ذخیره شده و به سرویس‌های پایین‌دستی مانند `extractPdfText` یا `analyzeWithGeminiFileAPI` ارسال می‌شود که می‌تواند منجر به رفتار غیرمنتظره، حملات DoS یا حتی اجرای کد در صورت وجود آسیب‌پذیری در پردازشگرهای فایل شود.
...
- `backend/middleware/upload.js:36-45` — `upload.fileFilter` — منطق فیلتر کردن فایل که بر اساس `file.mimetype` کلاینت عمل می‌کند.
  ```jsx
...
Node.js/Express backend، Multer برای آپلود فایل، FFmpeg و Gemini File API برای پردازش فایل.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/routes/index.js` (سطر 53) — این فایل از middleware `upload` برای مسیرهای آپلود فایل استفاده می‌کند.
- `backend/services/analysisService.js` (سطر 42) — این سرویس فایل‌های آپلود شده را برای تحلیل دریافت می‌کند و ممکن است با فایل‌های با نوع MIME جعلی مواجه شود.

## 🌐 نقشهٔ وابستگی‌ها
این تغییر بر تمام مسیرهای آپلود فایل تأثیر می‌گذارد و نیازمند افزودن یک مرحلهٔ اعتبارسنجی جدید قبل از ذخیرهٔ فایل یا ارسال آن به سرویس‌های تحلیل است.
...
- [ ] سیستم آپلود فایل قادر به تشخیص و رد فایل‌هایی با نوع MIME جعلی باشد.
- [ ] یک تست واحد جدید برای سناریوهای جعل نوع MIME اضافه شود.
- [ ] عملکرد آپلود فایل‌های مجاز بدون تغییر باقی بماند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. برای جلوگیری از جعل نوع MIME، باید یک بررسی سرور-ساید برای تأیید نوع واقعی محتوای فایل (content-type sniffing) پیاده‌سازی شود. این کار را می‌توان با استفاده از کتابخانه‌هایی مانند `file-type` در Node.js انجام داد که محتوای باینری فایل را برای تشخیص نوع واقعی آن بررسی می‌کنند، نه اینکه صرفاً به هدر `Content-Type` ارسالی از کلاینت اعتماد کنند.

## 💡 نمونه‌های قبل/بعد
**اعتبارسنجی نوع فایل با content-type sniffing**
...
_بعد:_
```
const detectedMimeType = await fileTypeFromFile(filePath); // استفاده از کتابخانه file-type
if (detectedMimeType && isAllowedMimeType(detectedMimeType.mime)) { cb(null, true); } else { /* خطا */ }
```
...
## 🧪 دستورات اعتبارسنجی
- `npm run test -- upload`
- `تست دستی آپلود فایل‌های با نوع MIME جعلی`

## ⚠️ ریسک‌ها و موارد احتیاط
افزایش زمان پردازش آپلود فایل به دلیل نیاز به خواندن بخشی از فایل برای تشخیص نوع آن؛ احتمال خطای تشخیص نوع فایل برای فرمت‌های خاص.
...
1. سیستم آپلود فایل قادر به تشخیص و رد فایل‌هایی با نوع MIME جعلی باشد. _(verify: backend_test)_
2. یک تست واحد جدید برای سناریوهای جعل نوع MIME اضافه شود. _(verify: backend_test)_
3. عملکرد آپلود فایل‌های مجاز بدون تغییر باقی بماند. _(verify: api_response)_
