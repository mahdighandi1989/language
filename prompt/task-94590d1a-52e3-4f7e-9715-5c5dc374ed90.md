---
task_id: 94590d1a-52e3-4f7e-9715-5c5dc374ed90
title: 'تناقض contract بین endpoint های Telegram: index.js مسیرها را register می‌کند ولی routes/index.js آن‌ها را نمی‌شناسد'
type: refactor
priority: medium
...
created_at: '2026-06-06T07:57:19.290555+00:00'
updated_at: '2026-06-06T08:04:15.216348+00:00'
target_files:
- backend/services/telegram/index.js
- backend/server.js
---

# تناقض contract بین endpoint های Telegram: index.js مسیرها را register می‌کند ولی routes/index.js آن‌ها را نمی‌شناسد

## Raw Idea

مسیرهای Telegram (POST /api/telegram/webhook, POST /api/telegram/link, GET /api/telegram/status) مستقیماً در backend/services/telegram/index.js روی app ثبت می‌شوند (خطوط 89-124)، در حالی که سایر مسیرهای /api در backend/routes/index.js به‌صورت متمرکز اسمبل می‌شوند. این یک ناسازگاری معماری است: لایهٔ routes تنها منبع حقیقت مسیرها نیست و بخشی از سطح API در لایهٔ services پنهان شده. علاوه بر این، attachTelegram در server.js (خط 87) *قبل* از app.use('/api', generalLimiter) (خط 95) فراخوانی می‌شود، پس مسیرهای telegram تحت rate limiter عمومی /api نیستند — این رفتار در کامنت خط 86 عمداً توضیح داده شده ولی یک ناسازگاری ضمنی با انتظار «همهٔ /api تحت rate limit» ایجاد می‌کند.
...
- `backend/services/telegram/index.js:83-128` — `attachTelegram` — مسیرهای /api/telegram/* در لایهٔ services ثبت می‌شوند نه routes
  ```jsx
  ...
  ```
- `backend/server.js:86-95` — `attachTelegram call` — attachTelegram قبل از generalLimiter صدا زده می‌شود، پس telegram routes rate-limit نمی‌شوند
  ```jsx
  attachTelegram(app, {...});
...
Node/Express ESM، layered architecture (routes/controllers/services barrels)

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/routes/index.js` (سطر 34) — لایهٔ متمرکز routes که telegram را شامل نمی‌شود
- `backend/middleware/firebaseAuth.js` (سطر 41) — requireAuth که در /api/telegram/link استفاده می‌شود
- `backend/services/telegram/config.js` — `index.js` این فایل را import می‌کند
- `backend/services/telegram/client.js` — `index.js` این فایل را import می‌کند
- `backend/services/telegram/store.js` — `index.js` این فایل را import می‌کند
- `backend/services/telegram/logger.js` — `index.js` این فایل را import می‌کند
...
## 🌐 نقشهٔ وابستگی‌ها
attachTelegram توسط server.js صدا زده می‌شود و سه مسیر /api را روی app ثبت می‌کند؛ هر تغییر باید با لایهٔ routes و ترتیب middleware در server.js هماهنگ بماند.
...
- [ ] همهٔ مسیرهای /api در یک‌جا (routes layer) قابل کشف باشند یا تصمیم خلاف آن در routes/index.js مستند شده باشد
- [ ] رفتار rate-limit مسیرهای telegram (آیا تحت generalLimiter هست یا نه) به‌صراحت در کامنت یا docs ثبت شود
- [ ] تست‌های موجود backend/tests/test_telegram_service.js همچنان pass شوند
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. مسیرهای HTTP تلگرام را از services/telegram/index.js به یک router جدا (مثلاً backend/routes/telegramRoutes.js) منتقل کن و در backend/routes/index.js mount کن، یا حداقل در README/docs ثبت کن که این مسیرها عمداً خارج از apiRouter و خارج از generalLimiter هستند. اگر rate limiting برای webhook لازم نیست، این تصمیم را در یک کامنت در routes/index.js مستند کن تا کسی دنبال مسیر telegram در routes نگردد.

## 💡 نمونه‌های قبل/بعد
**انتقال به routes layer**
...
telegramRouter.post('/api/telegram/webhook', ...)
// routes/index.js
...
## 🧪 دستورات اعتبارسنجی
- `node --test backend/tests/test_telegram_service.js`
- `python -m pytest tests/test_server_startup.py`

## ⚠️ ریسک‌ها و موارد احتیاط
جابجایی مسیرها می‌تواند ترتیب middleware (requireAuth, generalLimiter) را تغییر دهد؛ نیاز به تست دقیق webhook secret و link flow
...
1. همهٔ مسیرهای /api در یک‌جا (routes layer) قابل کشف باشند یا تصمیم خلاف آن در routes/index.js مستند شده باشد _(verify: static)_
2. رفتار rate-limit مسیرهای telegram (آیا تحت generalLimiter هست یا نه) به‌صراحت در کامنت یا docs ثبت شود _(verify: static)_
3. تست‌های موجود backend/tests/test_telegram_service.js همچنان pass شوند _(verify: backend_test)_
