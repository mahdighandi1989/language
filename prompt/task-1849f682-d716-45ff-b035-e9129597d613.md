---
task_id: 1849f682-d716-45ff-b035-e9129597d613
title: notification بدون event_type صریح در test_outcome_metrics.py
...
created_at: '2026-06-06T07:59:11.507448+00:00'
updated_at: '2026-06-13T08:13:37.584488+00:00'
target_files:
- tests/e2e/test_outcome_metrics.py
---

# notification بدون event_type صریح در test_outcome_metrics.py

## Raw Idea

## 📋 شرح
در `tests/e2e/test_outcome_metrics.py` یک `notify_event` call پیدا شد که اولین پارامتر (event_type) رشته‌ای ساده/مبهم است یا خالی.
...
---
[scan #2 at 2026-06-13T08:12:02.494381+00:00]
فراخوانی `notify_event` به صراحت `silent=True` را تنظیم می‌کند، اما `assert note.silent is False` پس از آن نشان می‌دهد که انتظار می‌رود نوتیفیکیشن *غیرفعال* (non-silent) باشد. این یک تناقض حیاتی است. برای یک رویداد `verify_failed` با اولویت `high`، غیرفعال بودن مناسب است، اما این ناهماهنگی بین پارام
---
[scan #3 at 2026-06-13T08:13:37.584480+00:00]
## 📋 شرح
در `tests/e2e/test_outcome_metrics.py` یک `notify_event` call پیدا شد که اولین پارامتر (event_type) رشته‌ای ساده/مبهم است یا خالی.
...
- `tests/e2e/test_outcome_metrics.py:127`
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile tests/e2e/test_outcome_metrics.py`
- `ruff check tests/e2e/test_outcome_metrics.py`
- `pytest -x`
