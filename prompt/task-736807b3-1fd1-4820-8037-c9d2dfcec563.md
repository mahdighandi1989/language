---
task_id: 736807b3-1fd1-4820-8037-c9d2dfcec563
title: validate() فقط req.body را اعتبارسنجی می‌کند و در /api/upload و /api/audio/process هیچ schema validation روی فایل/فیلدها وجود ندارد (security boundary)
type: security
priority: medium
...
created_at: '2026-06-06T07:57:45.033724+00:00'
updated_at: '2026-06-06T07:57:45.033727+00:00'
---

# validate() فقط req.body را اعتبارسنجی می‌کند و در /api/upload و /api/audio/process هیچ schema validation روی فایل/فیلدها وجود ندارد (security boundary)

## Raw Idea

middleware validate در validate.js فقط schema.parse(req.body) را اجرا می‌کند. مسیرهای /api/upload و /api/audio/process (routes/index.js خطوط 65-82) هیچ validate() ندارند و فقط به fileFilter در multer (isAllowedMimeType) تکیه می‌کنند. fileFilter بر اساس file.mimetype اعلام‌شده توسط کلاینت تصمیم می‌گیرد که قابل جعل است (client-controlled MIME). همچنین filename در storage از `${Date.now()}-${file.originalname}` ساخته می‌شود (upload.js خط 16) و originalname می‌تواند شامل مسیر یا کاراکترهای path traversal مثل ../ باشد — هرچند join با uploadDir ریسک را کاهش می‌دهد، originalname خام به‌عنوان نام فایل روی دیسک نوشته می‌شود و در پاسخ‌ها/لاگ‌ها بازتاب می‌یابد و یک XSS/path vector بالقوه است. هیچ sanitization روی originalname دیده نمی‌شود.
...
1. originalname را در filename callback پاک‌سازی کن (حذف path separators و
