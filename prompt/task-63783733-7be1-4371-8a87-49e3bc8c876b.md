---
task_id: 63783733-7be1-4371-8a87-49e3bc8c876b
title: Duplicated CORS Origin Logic in `server.js` and `security.js`
type: refactor
priority: medium
...
created_at: '2026-06-13T08:12:02.622546+00:00'
updated_at: '2026-06-13T08:19:17.581248+00:00'
target_files:
- backend/server.js
- backend/middleware/security.js
---

# Duplicated CORS Origin Logic in `server.js` and `security.js`

## Raw Idea

The logic for constructing the list of `allowedOrigins` for CORS is duplicated in `backend/server.js` (lines 47-53) and `backend/middleware/security.js` (lines 19-29, `buildAllowedOrigins` function). Both snippets perform the same operation of combining `CORS_ORIGIN`, `FRONTEND_URL`, and `DEV_ORIGIN` into a de-duplicated array. This redundancy creates a maintenance burden and a risk of inconsistencies if the logic needs to be updated in the future.
...
- `backend/server.js:47-53` — Duplicated CORS origin construction logic.
  ```jsx
  ...
  ```
- `backend/middleware/security.js:19-29` — `buildAllowedOrigins` — The canonical `buildAllowedOrigins` function.
  ```jsx
...
- `backend/middleware/index.js` — Exports `applySecurity`, which is related to security middleware.
...
## 🌐 نقشهٔ وابستگی‌ها
This affects the core CORS configuration of the Express server.
...
- [ ] `backend/server.js` uses `buildAllowedOrigins` from `backend/middleware/security.js`.
- [ ] The `cors` middleware in `backend/server.js` continues to function correctly.
- [ ] CORS behavior remains unchanged for allowed and disallowed origins.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Remove the duplicated logic from `backend/server.js`. Instead, `backend/server.js` should import and use the `buildAllowedOrigins` function directly from `backend/middleware/security.js` to ensure a single, authoritative source for CORS origin configuration.

## 💡 نمونه‌های قبل/بعد
**`backend/server.js` `allowedOrigins` definition**
...
Minor risk of misconfiguring CORS if the import path or usage is incorrect.
...
1. `backend/server.js` uses `buildAllowedOrigins` from `backend/middleware/security.js`. _(verify: static)_
2. The `cors` middleware in `backend/server.js` continues to function correctly. _(verify: api_response)_
3. CORS behavior remains unchanged for allowed and disallowed origins. _(verify: api_response)_
