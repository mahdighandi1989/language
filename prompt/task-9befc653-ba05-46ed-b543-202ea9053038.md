---
task_id: 9befc653-ba05-46ed-b543-202ea9053038
title: duplicate تعریف پرامپت‌های Lebanese live-voice در backend/models/prompts.js و frontend/src/components/App.jsx
type: refactor
priority: medium
...
created_at: '2026-06-06T07:57:19.307380+00:00'
updated_at: '2026-06-06T08:04:22.418300+00:00'
target_files:
- backend/models/prompts.js
- frontend/src/components/App.jsx
---

# duplicate تعریف پرامپت‌های Lebanese live-voice در backend/models/prompts.js و frontend/src/components/App.jsx

## Raw Idea

مجموعهٔ کاملی از پرامپت‌های live-voice (liveVoiceIntro, liveVoiceRules, liveVoicePronunciation, liveVoiceVocabulary, liveVoiceVerbs, liveVoiceExpressions, liveVoiceExamples, liveVoiceStyle, liveVoiceImportantRules, liveVoiceClosing) به‌صورت کلمه‌به‌کلمه یکسان در دو جای کاملاً جدا تعریف شده: backend/models/prompts.js (خطوط 113-202، defaultLivePrompts) و frontend/src/components/App.jsx (خطوط 61-276، defaultPrompts). همچنین قواعد تصحیح/تحلیل (LEBANESE_CORRECTION_PROMPT و ANALYSIS_SYSTEM_PROMPT) در بک‌اند با analysisLebaneseCorrection/analysisOutputFormat فرانت‌اند هم‌پوشانی محتوایی دارند. این duplication یعنی هر تغییر در قواعد لهجه باید در دو فایل به‌روزرسانی شود و خطر drift (ناهماهنگ شدن نسخه‌ها) بالاست.
...
- `backend/models/prompts.js:113-202` — `defaultLivePrompts` — تعریف بک‌اند پرامپت‌های live
  ```jsx
  export const defaultLivePrompts = {
    liveVoiceIntro: `انت جاد، شب لبناني من بيروت عمرك 28 سنة...`,
    liveVoiceRules: `مهم كتير كتير: لازم تحكي باللهجة اللبنانية البيروتية بس!...`,
    ...
  ```
- `frontend/src/components/App.jsx:61-276` — `defaultPrompts` — تعریف فرانت‌اند همان پرامپت‌ها (یکسان کلمه‌به‌کلمه)
  ```jsx
...
Node/Express + React (Vite)؛ پرامپت‌ها به‌صورت string literal در دو زبان

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/services/prompts.js` — re-export پرامپت‌های models که service-layer از آن استفاده می‌کند
- `backend/services/analysisService.js` (سطر 3) — ANALYSIS_SYSTEM_PROMPT را import و مصرف می‌کند
...
## 🌐 نقشهٔ وابستگی‌ها
پرامپت‌های live در بک‌اند توسط liveProxyService و در فرانت‌اند توسط Settings UI مصرف می‌شوند؛ این دو نسخه باید همگام بمانند ولی هیچ مکانیزمی این تضمین را نمی‌دهد.
...
- [ ] یک منبع واحد یا یک تست consistency برای پرامپت‌های مشترک live-voice وجود داشته باشد
- [ ] تغییر در یک پرامپت live در بک‌اند باعث شکست تست شود اگر فرانت‌اند همگام نشود (یا برعکس)
- [ ] هیچ کلید live-voice پرامپتی در یک طرف باشد که در طرف دیگر نباشد
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. یک منبع واحد حقیقت برای پرامپت‌های پیش‌فرض بساز. چون بک‌اند پرامپت‌های live را به Gemini Live ارسال می‌کند و فرانت‌اند آن‌ها را به‌عنوان مقادیر پیش‌فرض قابل ویرایش در Settings نشان می‌دهد، یک endpoint مثل GET /api/prompts/defaults اضافه کن که defaultLivePrompts بک‌اند را برگرداند، یا پرامپت‌ها را در یک پکیج/فایل JSON مشترک نگه‌داری کن. حداقل: یک تست consistency بنویس که تطابق کلیدها و محتوای پرامپت‌های مشترک بین دو فایل را verify کند تا drift زود کشف شود.
...
_قبل:_
```
// no shared source, two copies drift independently
```

_بعد:_
```
// tests assert frontend defaultPrompts.liveVoiceIntro === backend defaultLivePrompts.liveVoiceIntro
```
...
اگر endpoint مشترک اضافه شود، فرانت‌اند به یک fetch اضافه در زمان بارگذاری Settings نیاز دارد؛ باید fallback به مقادیر local حفظ شود
...
1. یک منبع واحد یا یک تست consistency برای پرامپت‌های مشترک live-voice وجود داشته باشد _(verify: backend_test)_
2. تغییر در یک پرامپت live در بک‌اند باعث شکست تست شود اگر فرانت‌اند همگام نشود (یا برعکس) _(verify: backend_test)_
3. هیچ کلید live-voice پرامپتی در یک طرف باشد که در طرف دیگر نباشد _(verify: backend_test)_
