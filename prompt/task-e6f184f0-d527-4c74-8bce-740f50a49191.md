---
task_id: e6f184f0-d527-4c74-8bce-740f50a49191
...
created_at: '2026-06-06T07:55:56.713193+00:00'
updated_at: '2026-06-13T08:10:56.693209+00:00'
target_files:
- tests/unit/_vocabulary.py
---
...
The logic for handling word text case-sensitivity is inconsistent across methods. `add_word` performs a case-sensitive check for duplicate words (`w["text"] == normalized`), meaning 'Apple' and 'apple' can be added as distinct words. However, `search_words` performs a case-insensitive search (`needle in w["text"].lower()`), meaning a search for 'apple' would find both 'Apple' and 'apple'. This dis

📁 file: tests/unit/_vocabulary.py (line 27)

🎯 پیشنهاد: این الگو معمولاً منطق سیستم را در شرایط لبه می‌شکند.
---
[scan #2 at 2026-06-13T08:10:56.693200+00:00]
The ambiguity in the `default_status` contract (as detailed in the 'Under-engineering' issue) creates a risk of a new conditional inconsistency. The final HTTP status code returned by the terminal error handler might vary based on the Node.js implementation's interpretation of the contract, potentia
...
- `tests/unit/test_vocabulary.py` — این فایل `_vocabulary.py` را import می‌کند (caller)
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile tests/unit/_vocabulary.py`
- `ruff check tests/unit/_vocabulary.py`
- `pytest -x`
