---
task_id: 2bae37f5-4736-4ccc-bc3d-999e9d81bf35
...
created_at: '2026-06-13T08:12:03.362031+00:00'
updated_at: '2026-06-13T08:12:03.362033+00:00'
target_files:
- tests/_build_helpers.py
---
...
In `ensure_node_modules`, the output of `npm install` is captured (`capture_output=True`) but not explicitly logged or used for debugging purposes, even on success. While `check=True` raises an exception on failure, providing the `stderr` in the exception message or logging it would significantly improve the feedback loop for CI debugging.

📁 file: tests/_build_helpers.py (line 31)
...
- `tests/test_build.py` — این فایل `_build_helpers.py` را import می‌کند (caller)
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile tests/_build_helpers.py`
- `ruff check tests/_build_helpers.py`
- `pytest -x`
