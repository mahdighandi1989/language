---
task_id: 39b93db2-1012-46af-98d0-20b5ad9f5597
title: Inconsistent Error Response Structure for 400-level Errors
type: refactor
priority: medium
...
created_at: '2026-06-13T08:12:02.667724+00:00'
updated_at: '2026-06-13T08:19:32.761296+00:00'
target_files:
- backend/middleware/upload.js
- backend/middleware/validate.js
---

# Inconsistent Error Response Structure for 400-level Errors

## Raw Idea

The backend handles 400-level errors (Bad Request) with inconsistent JSON response structures. Specifically, `backend/middleware/upload.js` (`handleMulterError`) returns an object with an `error` string (e.g., `{ error: 'حجم فایل بیش از حد مجاز است (حداکثر 500MB)' }`), while `backend/middleware/validate.js` (`validate`) returns an object with an `error` string and a `details` array (e.g., `{ error: 'ورودی نامعتبر است', details: [{ path: 'field', message: 'required' }] }`). This inconsistency makes it harder for frontend clients to parse and display error messages uniformly.
...
- `backend/middleware/upload.js:48-59` — `handleMulterError` — Multer error responses lack a `details` array.
  ```jsx
  ...
  ```
- `backend/middleware/validate.js:13-17` — `validate` — Zod validation errors include a `details` array.
  ```jsx
...
- `backend/routes/index.js` — Uses both `handleMulterError` and `validate` middleware.
- `backend/services/analysisService.js` — این فایل `upload.js` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
This affects how the frontend handles and displays validation and upload-related errors.
...
- [ ] All 400-level error responses from the backend follow a consistent JSON structure (e.g., always include `error` string and an optional `details` array).
- [ ] Frontend error handling can uniformly process these responses.
- [ ] Unit/integration tests for error paths confirm the new structure.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Standardize the JSON error response structure for all 400-level errors. A common pattern is to always include a `message` and optionally a `details` array for validation errors. Refactor `handleMulterError` to align with the `validate` middleware's structure, or create a common error utility to ensure consistency.

## 💡 نمونه‌های قبل/بعد
**Multer `LIMIT_FILE_SIZE` error response**
...
```
return res.status(400).json({
  error: 'خطای آپلود فایل',
  details: [{ path: 'file', message: 'حجم فایل بیش از حد مجاز است (حداکثر 500MB)' }]
});
```
...
## 🧪 دستورات اعتبارسنجی
- `npm run test -- backend/tests/test_upload_validation.js`

## ⚠️ ریسک‌ها و موارد احتیاط
Minor risk of breaking existing frontend error parsing if not communicated.
...
1. All 400-level error responses from the backend follow a consistent JSON structure (e.g., always include `error` string and an optional `details` array). _(verify: api_response)_
2. Frontend error handling can uniformly process these responses. _(verify: ui_interaction)_
3. Unit/integration tests for error paths confirm the new structure. _(verify: backend_test)_
