---
task_id: 08014f43-476a-409a-bfbe-8d611e64d160
title: پیاده‌سازی مدیریت خطا برای WebSocket و پیام‌ها
type: bug
priority: high
...
created_at: '2026-05-30T07:41:50.946010+00:00'
updated_at: '2026-06-02T10:59:29.252975+00:00'
target_files:
- backend/server.js
- frontend/src/App.jsx
---

# پیاده‌سازی مدیریت خطا برای WebSocket و پیام‌ها

## Raw Idea

در backend/server.js خط 45، WebSocket server ایجاد شده اما هیچ event handler برای 'error' و 'unexpected-response' روی WebSocket client یا server تعریف نشده است. در frontend/src/App.jsx خطوط 16-38، تابع connectWS() فقط 'onclose' را مدیریت می‌کند اما 'onerror' را نادیده می‌گیرد (خط 37: `ws.onerror = () => {};`). این باعث می‌شود خطاهای اتصال WebSocket (مانند قطع ناگهانی شبکه، خطای SSL، یا timeout) بی‌صدا نادیده گرفته شوند و کاربر هیچ بازخوردی دریافت نکند. همچنین در خط 35، `ws.onclose` با `setTimeout(connectWS, 3000)` تلاش مجدد می‌کند اما بدون محدودیت (exponential backoff) که می‌تواند باعث flooding سرور شود.
...
عدم مدیریت خطا در WebSocket connection و پیام‌های دریافتی

## 📍 موقعیت دقیق در پروژه
_(file:line — symbol — snippet)_

- `backend/server.js:45` — `WebSocketServer` — بعد از این خط، باید wss.on('error', ...) اضافه شود
  ```jsx
  const wss = new WebSocketServer({ server, path: '/ws/live' });
  ```
- `frontend/src/App.jsx:16-38` — `connectWS` — ws.onerror خالی است و reconnect بدون backoff
  ```jsx
  const connectWS = () => {
...
Node.js + ws library + React

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `frontend/src/App.jsx` (سطر 8) — Inspector Bridge WebSocket client
- `backend/server.js` (سطر 45) — WebSocket server

## 🌐 نقشهٔ وابستگی‌ها
WebSocket server در backend و client در frontend هر دو نیاز به اصلاح دارند.
...
1. 1. به WebSocket server در backend/server.js یک event handler برای 'error' اضافه کن که خطا را لاگ کند. 2. در frontend/src/App.jsx، تابع connectWS را اصلاح کن: (a) handler 'onerror' را با لاگ کردن خطا و نمایش پیام به کاربر جایگزین کن. (b) برای reconnect از exponential backoff استفاده کن (مثلاً 1s, 2s, 4s, 8s, max 30s). (c) یک state برای نمایش وضعیت اتصال به کاربر اضافه کن.
...
_بعد:_
```
ws.onerror = (err) => { console.error('WebSocket error:', err); setConnectionStatus('error'); };
```
