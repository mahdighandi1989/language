---
task_id: 750b306e-cd4c-4035-b6de-62956f02ebaa
title: '[منطق] Silent parameter contradicts critical event alerting purpose'
type: logic_audit
priority: critical
...
created_at: '2026-06-13T08:08:31.739969+00:00'
updated_at: '2026-06-13T08:08:31.739972+00:00'
---

# [منطق] Silent parameter contradicts critical event alerting purpose
...
The `backend/app/notifications.py` module is explicitly designed to "emit user-facing alerts for critical backend events, ensuring immediate operator awareness of outages." However, it accepts a `silent (bool)` input parameter. For an event to be truly 'critical' and require 'immediate operator awareness,' it should never be silently suppressed. Allowing a critical alert to be optionally silenced directly undermines the module's stated primary purpose and the reliability of the notification pipe

## 💥 پیامد (impact)
Critical backend events might go unnoticed by operators if the `silent` flag is misused or inadvertently set to `True`. This could lead to prolonged outages, data loss, or other severe system failures due to delayed response and lack of immediate intervention, directly contradicting the goal of 'imm

## 🛠 پیشنهاد رفع اولیه
Re-evaluate the necessity and behavior of the `silent` parameter within `notifications.py`. For notifications concerning critical events, the `silent` option should either be removed entirely, or its functionality should be strictly controlled (e.g., only allowing `silent=False` for critical priorities, or renaming it to `log_only` if it's meant to suppress external alerts but still log). If `sile
...
### Step 1: ارزیابی مجدد پارامتر `silent` در ماژول `notifications.py`
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی عمیق ضرورت و رفتار فعلی پارامتر `silent` در ماژول `backend/app/notifications.py` است. هدف، درک کامل نحوه استفاده از این پارامتر در حال حاضر، شناسایی تمام نقاطی که فراخوانی می‌شود، و تعیین اینکه آیا استفاده فعلی با هدف اصلی ماژول (هشدار فوری برای رویدادهای حیاتی) همخوانی دارد یا خیر، می‌باشد. این مرحله شامل هیچ تغییر کدی نیست، بلکه یک فاز تحلیلی برای جمع‌آوری اطلاعات و تصمیم‌گیری در مورد بهترین رویکرد اصلاحی است.
**Excerpt:**
```
Re-evaluate the necessity and behavior of the `silent` parameter within `notifications.py`. For an event to be truly 'critical' and require 'immediate operator awareness,' it should never be silently suppressed. Allowing a critical alert to be optionally silenced directly undermines the module's stated primary purpose and the reliability of the notification pipe
```

### Step 2: پیاده‌سازی گزینه ۱: حذف کامل پارامتر `silent` برای رویدادهای حیاتی
**Status:** `pending` (0%)
**Scope:** این مرحله شامل اصلاح ماژول `backend/app/notifications.py` برای حذف کامل پارامتر `silent` یا نادیده گرفتن آن برای هر نوع نوتیفیکیشن که به عنوان 'حیاتی' (critical) طبقه‌بندی می‌شود. هدف این است که اطمینان حاصل شود هیچ رویداد حیاتی هرگز به صورت 'silent' سرکوب نشود. این کار ممکن است شامل تغییر signature توابع، به‌روزرسانی تمام فراخوانی‌های مربوطه در codebase، و افزودن منطق برای شناسایی رویدادهای حیاتی باشد.
**Excerpt:**
```
For notifications concerning critical events, the `silent` option should either be removed entirely, or its functionality should be strictly controlled (e.g., only allowing `silent=False` for critical priorities, or renaming it to `log_only` if it's meant to suppress external alerts but still log). If `sile
```

### Step 3: پیاده‌سازی گزینه ۲: کنترل دقیق پارامتر `silent` برای رویدادهای حیاتی
**Status:** `pending` (0%)
**Scope:** این مرحله شامل اصلاح ماژول `backend/app/notifications.py` برای اعمال کنترل‌های سختگیرانه بر روی پارامتر `silent` برای رویدادهای حیاتی است. به جای حذف کامل، این گزینه بر محدود کردن عملکرد `silent` تمرکز دارد؛ به عنوان مثال، اطمینان از اینکه `silent=False` تنها مقدار مجاز برای نوتیفیکیشن‌های با اولویت حیاتی باشد. این ممکن است شامل افزودن اعتبارسنجی (validation) در زمان اجرا یا تغییر منطق داخلی برای نادیده گرفتن `silent=True` در صورت تشخیص یک رویداد حیاتی باشد.
**Excerpt:**
```
For notifications concerning critical events, the `silent` option should either be removed entirely, or its functionality should be strictly controlled (e.g., only allowing `silent=False` for critical priorities, or renaming it to `log_only` if it's meant to suppress external alerts but still log).
```

### Step 4: پیاده‌سازی گزینه ۳: تغییر نام پارامتر `silent` به `log_only`
**Status:** `pending` (0%)
**Scope:** این مرحله شامل تغییر نام پارامتر `silent` به `log_only` در ماژول `backend/app/notifications.py` و به‌روزرسانی منطق مربوطه است. هدف این است که نام پارامتر به وضوح نشان دهد که عملکرد آن صرفاً سرکوب هشدارهای *خارجی* است، در حالی که اطمینان حاصل شود که رویداد همچنان *لاگ* می‌شود. این تغییر شامل به‌روزرسانی تمام فراخوانی‌های داخلی و خارجی به این ماژول برای استفاده از نام پارامتر جدید و اطمینان از اینکه لاگ‌برداری همیشه انجام می‌شود، حتی اگر هشدار خارجی سرکوب شود.
**Excerpt:**
```
For notifications concerning critical events, the `silent` option should either be removed entirely, or its functionality should be strictly controlled (e.g., only allowing `silent=False` for critical priorities, or renaming it to `log_only` if it's meant to suppress external alerts but still log).
```
