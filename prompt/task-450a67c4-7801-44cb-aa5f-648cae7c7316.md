---
task_id: 450a67c4-7801-44cb-aa5f-648cae7c7316
title: ریسک تزریق پرامپت از طریق دستورات کاربر در سرویس تحلیل
type: security
priority: medium
...
created_at: '2026-06-06T07:57:19.264967+00:00'
updated_at: '2026-06-06T08:03:50.996432+00:00'
target_files:
- backend/services/analysisService.js
---

# ریسک تزریق پرامپت از طریق دستورات کاربر در سرویس تحلیل

## Raw Idea

در فایل `backend/services/analysisService.js`، دستورات ارسالی توسط کاربر (`userInstructions`) مستقیماً به `ANALYSIS_SYSTEM_PROMPT` اضافه می‌شوند. اگرچه `ANALYSIS_SYSTEM_PROMPT` در ابتدا قرار می‌گیرد، اما یک مهاجم با دستکاری `userInstructions` می‌تواند سعی در دور زدن یا تغییر رفتار مدل LLM داشته باشد. این امر می‌تواند منجر به تولید محتوای ناخواسته یا افشای اطلاعات شود، هرچند که ماهیت فعلی وظیفه (تحلیل) ممکن است دامنهٔ آسیب را محدود کند.
...
- `backend/services/analysisService.js:37` — `analyzeUploads` — نقطهٔ ادغام دستورات کاربر با پرامپت سیستمی
  ```jsx
...
Node.js/Express backend، Google Gemini LLM، پرامپت‌های متنی.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/controllers/analysisController.js` (سطر 27) — این کنترلر تابع `analyzeUploads` را فراخوانی می‌کند که ورودی کاربر را به آن ارسال می‌کند.
- `backend/routes/index.js` (سطر 58) — مسیر `/api/analyze-files` از `analysisController.analyzeFiles` استفاده می‌کند.
- `backend/services/prompts.js` — `analysisService.js` این فایل را import می‌کند
- `backend/services/pdfService.js` — `analysisService.js` این فایل را import می‌کند
- `backend/services/videoService.js` — `analysisService.js` این فایل را import می‌کند
- `backend/utils/chunking.js` — `analysisService.js` این فایل را import می‌کند

## 🌐 نقشهٔ وابستگی‌ها
این تغییر بر نحوهٔ پردازش دستورات کاربر در پایپ‌لاین تحلیل LLM تأثیر می‌گذارد و نیازمند هماهنگی با `analysisController` و `geminiService` است.
...
- [ ] ورودی‌های مخرب در `userInstructions` نتوانند پرامپت سیستمی را دور بزنند یا تغییر دهند.
- [ ] یک تابع پاکسازی جدید برای ورودی‌های پرامپت کاربر اضافه شود.
- [ ] تست واحد جدید برای سناریوهای تزریق پرامپت اضافه شود.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. برای کاهش ریسک تزریق پرامپت، `userInstructions` باید قبل از ارسال به مدل LLM، به طور دقیق‌تر پاکسازی (sanitize) شود. می‌توان از یک روش templating ساختاریافته استفاده کرد که ورودی کاربر را به وضوح از دستورالعمل‌های سیستمی جدا کند، یا از یک LLM محافظ (guardrail LLM) برای اعتبارسنجی دستورات کاربر قبل از ادغام استفاده کرد.

## 💡 نمونه‌های قبل/بعد
**نمونهٔ پاکسازی پرامپت**
...
_بعد:_
```
const sanitizedUserInstructions = sanitizePromptInput(userInstructions); // تابع جدید برای پاکسازی
const systemPrompt = ANALYSIS_SYSTEM_PROMPT + (sanitizedUserInstructions ? `\n**دستورات کاربر:** ${sanitizedUserInstructions}` : '');
```
...
## 🧪 دستورات اعتبارسنجی
- `pytest tests/backend/ai_llm/test_pipeline_logic.py`
- `npm run test -- analysisService`

## ⚠️ ریسک‌ها و موارد احتیاط
پیچیدگی در پیاده‌سازی پاکسازی مؤثر پرامپت بدون مسدود کردن دستورات مشروع کاربر؛ احتمال تغییر در خروجی LLM.
...
1. ورودی‌های مخرب در `userInstructions` نتوانند پرامپت سیستمی را دور بزنند یا تغییر دهند. _(verify: backend_test)_
2. یک تابع پاکسازی جدید برای ورودی‌های پرامپت کاربر اضافه شود. _(verify: static)_
3. تست واحد جدید برای سناریوهای تزریق پرامپت اضافه شود. _(verify: backend_test)_
