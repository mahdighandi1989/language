---
task_id: 2b76b4fc-5c61-4b69-adaa-d0226203816e
title: عدم مسدودسازی خودکار CI در صورت وجود آسیب‌پذیری‌های حیاتی وابستگی‌ها
type: security
priority: high
...
created_at: '2026-06-06T07:55:56.623899+00:00'
updated_at: '2026-06-06T08:01:24.672545+00:00'
target_files:
- package.json
---

# عدم مسدودسازی خودکار CI در صورت وجود آسیب‌پذیری‌های حیاتی وابستگی‌ها

## Raw Idea

اسکریپت `audit:ci` در `package.json` و همچنین workflow CI/CD در `docs/ci.workflow.example.yml`، دستور `npm audit --omit=dev --audit-level=critical` را با `|| true` اجرا می‌کنند. این بدان معناست که حتی اگر آسیب‌پذیری‌های حیاتی در وابستگی‌های production شناسایی شوند، pipeline CI/CD با خطا متوقف نمی‌شود و فقط گزارش می‌دهد. این رویکرد به جای مسدودسازی خودکار، بر پیگیری دستی و مداخله تکیه دارد که می‌تواند ریسک امنیتی را افزایش دهد.
...
- `package.json:20` — `audit:ci script` — اسکریپت `npm audit` که به صورت non-blocking اجرا می‌شود.
  ```json
...
Node.js/npm، GitHub Actions CI/CD.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `docs/ci.workflow.example.yml` (سطر 68) — این فایل workflow CI/CD را تعریف می‌کند که از این اسکریپت استفاده می‌کند.
- `docs/dependency_consistency.md` (سطر 1) — این سند هدف خروجی و معیارهای سازگاری وابستگی‌ها را توضیح می‌دهد.

## 🌐 نقشهٔ وابستگی‌ها
این تغییر بر فرآیند CI/CD و سیاست‌های مدیریت وابستگی تأثیر می‌گذارد و نیازمند هماهنگی تیمی برای پذیرش سیاست سخت‌گیرانه‌تر است.
...
- [ ] CI/CD در صورت یافتن آسیب‌پذیری‌های حیاتی در وابستگی‌های production، با خطا متوقف شود.
- [ ] تیم توسعه از سیاست جدید آگاه باشد و فرآیند رفع آسیب‌پذیری‌ها را دنبال کند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. برای افزایش امنیت، CI/CD باید به گونه‌ای پیکربندی شود که در صورت شناسایی آسیب‌پذیری‌های حیاتی در وابستگی‌های production، build را با خطا متوقف کند. `|| true` باید حذف شود تا `npm audit` در صورت یافتن آسیب‌پذیری‌های critical، CI را fail کند. این امر نیاز به یک فرآیند مدیریت آسیب‌پذیری قوی‌تر با SLAهای مشخص برای رفع سریع دارد.

## 💡 نمونه‌های قبل/بعد
**اجرای مسدودکنندهٔ npm audit**
...
افزایش تعداد fail شدن‌های CI/CD در ابتدا تا زمانی که تمام آسیب‌پذیری‌های حیاتی رفع شوند.
...
1. CI/CD در صورت یافتن آسیب‌پذیری‌های حیاتی در وابستگی‌های production، با خطا متوقف شود. _(verify: static)_
2. تیم توسعه از سیاست جدید آگاه باشد و فرآیند رفع آسیب‌پذیری‌ها را دنبال کند. _(verify: manual_only)_
