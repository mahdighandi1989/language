...
updated_at: '2026-06-01T19:30:57.201775+00:00'
archived: true
archived_at: '2026-06-01T19:30:57.201775+00:00'
tags:
- merged
---
...
### Step 1: بررسی و مستندسازی معماری فعلی pipeline ai_llm
**Status:** `partial` (50%)
**Scope:** این مرحله شامل بررسی کامل کدهای موجود مرتبط با ai_llm pipeline برای شناسایی کامپوننت‌ها، پرامپت‌ها، مدل‌های AI، و نحوه اتصال آن‌ها است. باید مشخص شود که آیا پرامپت‌ها به صورت متوالی، موازی، یا جایگزین اجرا می‌شوند. همچنین باید جایگاه دقیق پرامپت 'task_definition' در این pipeline مشخص شود. خروجی این مرحله یک سند معماری (نمودار یا توضیحات متنی) است که جریان کامل pipeline را نشان می‌دهد. این مرحله شامل پیاده‌سازی یا تغییر کد نیست، فقط مستندسازی وضعیت موجود است.
**Excerpt:**
```
یک نمودار یا توضیحات سطح بالا از معماری پایپ‌لاین ارائه دهید که توالی پرامپت‌ها، نقش مدل(های) AI و هر مرحله تجزیه خروجی یا اعتبارسنجی را نشان دهد. به وضوح تعریف کنید که پرامپت 'task_definition' چگونه با سایر پرامپت‌ها (که در حال حاضر تعریف نشده‌اند) مرتبط است.
```

### Step 2: تعریف جریان دقیق pipeline ai_llm با توالی پرامپت‌ها
**Status:** `partial` (50%)
**Scope:** بر اساس مستندات مرحله قبل، یک تعریف دقیق از جریان pipeline ارائه دهید. این شامل ترتیب اجرای پرامپت‌ها (متوالی، موازی، یا شرطی)، نحوه اتصال خروجی یک پرامپت به ورودی پرامپت بعدی، و نقش هر پرامپت در کل pipeline است. باید مشخص شود که پرامپت 'task_definition' در کجای این توالی قرار می‌گیرد و چه رابطه‌ای با سایر پرامپت‌ها دارد. این تعریف باید به صورت کد (مثلاً یک کلاس Pipeline یا یک تابع orchestrator) پیاده‌سازی شود.
**Excerpt:**
```
اطلاعات ارائه شده تنها لیستی از کامپوننت‌ها (پرامپت‌ها) است و نحوه اتصال این کامپوننت‌ها به یکدیگر، ترتیب اجرای آن‌ها، مدل AI که آن‌ها را مصرف می‌کند، یا نحوه پردازش خروجی‌های آن‌ها را توصیف نمی‌کند. مشخص نیست که آیا این پرامپت‌ها متوالی، موازی هستند یا برخی از آن‌ها فقط گزینه‌های جایگزین هستند.
```

### Step 3: تعریف نقش و جایگاه مدل(های) AI در pipeline
**Status:** `not_done` (0%)
**Scope:** مشخص کنید که کدام مدل AI (یا مدل‌ها) در pipeline استفاده می‌شوند، هر مدل کدام پرامپت‌ها را مصرف می‌کند، و چگونه خروجی مدل‌ها پردازش می‌شود. اگر چند مدل وجود دارد، نحوه تعامل آن‌ها (متوالی، موازی، یا ترکیبی) را تعریف کنید. همچنین مشخص کنید که آیا از یک مدل واحد برای همه پرامپت‌ها استفاده می‌شود یا مدل‌های مختلف برای پرامپت‌های مختلف.
**Excerpt:**
```
نقش مدل(های) AI و هر مرحله تجزیه خروجی یا اعتبارسنجی را نشان دهد.
```

### Step 4: تعریف مراحل تجزیه خروجی (output parsing) برای هر پرامپت
**Status:** `not_done` (0%)
**Scope:** برای هر پرامپت در pipeline، یک مرحله تجزیه خروجی تعریف کنید که خروجی خام مدل AI را به ساختار داده قابل استفاده تبدیل کند. این شامل تعریف schema خروجی مورد انتظار برای هر پرامپت، پیاده‌سازی parserهای مربوطه، و مدیریت خطاهای تجزیه (مثلاً خروجی JSON نامعتبر) است. همچنین مشخص کنید که خروجی تجزیه‌شده چگونه به پرامپت بعدی منتقل می‌شود.
**Excerpt:**
```
هر مرحله تجزیه خروجی یا اعتبارسنجی را نشان دهد.
```

### Step 5: تعریف مراحل اعتبارسنجی (validation) برای خروجی هر پرامپت
**Status:** `not_done` (0%)
**Scope:** برای هر پرامپت، یک مرحله اعتبارسنجی تعریف کنید که خروجی تجزیه‌شده را بررسی کند. این شامل بررسی نوع داده، محدوده مقادیر، وجود فیلدهای اجباری، و هرگونه محدودیت business logic است. همچنین باید مشخص شود که در صورت نامعتبر بودن خروجی، چه اقدامی انجام شود (تلاش مجدد، استفاده از مقدار پیش‌فرض، یا متوقف کردن pipeline).
**Excerpt:**
```
اعتبارسنجی و محافظت در برابر توهم (hallucination guards) بدون دانستن نحوه تعامل پ
```

### Step 6: یکپارچه‌سازی (integration) تمام مراحل در orchestrator اصلی
**Status:** `not_done` (0%)
**Scope:** تمام مراحل تعریف‌شده (orchestrator جریان، مدیریت مدل، تجزیه خروجی، اعتبارسنجی) را در یک pipeline واحد یکپارچه کنید. orchestrator اصلی باید بتواند یک ورودی دریافت کند، آن را از طریق توالی پرامپت‌ها پردازش کند، خروجی هر مرحله را تجزیه و اعتبارسنجی کند، و در نهایت خروجی نهایی را تولید کند. همچنین مدیریت خطا در سطح pipeline (مثلاً اگر یک مرحله failed شود) باید پیاده‌سازی شود.
**Excerpt:**
```
بدون یک جریان تعریف شده، 'پایپ‌لاین' تنها مجموعه‌ای از بخش‌های جدا از هم است. پیاده‌سازی یا اشکال‌زدایی سیستم غیرممکن است و هدف کلی بهبود ردیابی خطا نمی‌تواند به طور سیستماتیک محقق شود.
```

### Step 7: نوشتن تست‌های واحد (unit tests) برای orchestrator و جریان pipeline
**Status:** `not_done` (0%)
**Scope:** تست‌های واحد برای orchestrator اصلی و جریان کامل pipeline بنویسید. این تست‌ها باید سناریوهای مختلف را پوشش دهند: جریان عادی (happy path)، خطا در تجزیه خروجی، خطا در اعتبارسنجی، و خطا در فراخوانی مدل AI. همچنین تست‌هایی برای اطمینان از صحت توالی پرامپت‌ها و نحوه اتصال آن‌ها بنویسید.
**Excerpt:**
```
سوالات مربوط به سازگاری مدل AI، تجزیه خروجی و محافظت در برابر توهم (hallucination guards) بدون دانستن نحوه تعامل پ
```

### Step 8: نوشتن تست‌های یکپارچه‌سازی (integration tests) برای pipeline کامل
**Status:** `not_done` (0%)
**Scope:** تست‌های یکپارچه‌سازی بنویسید که pipeline کامل را با وابستگی‌های واقعی (یا mock شده) تست کند. این تست‌ها باید اطمینان حاصل کنند که تمام اجزای pipeline (orchestrator، parsers، validators، model manager) به درستی با یکدیگر کار می‌کنند. همچنین تست‌هایی برای سناریوهای خطای واقع‌گرایانه (مثلاً timeout مدل AI، خروجی نامعتبر) بنویسید.
**Excerpt:**
```
این کلاس bug ها در test معمولی پیدا نمی‌شوند چون unit test ها در silo اجرا می‌شوند.
```

### Step 9: بازبینی نهایی (audit) و مستندسازی coherence issue و رفع آن
**Status:** `partial` (50%)
**Scope:** یک بازبینی نهایی از کل pipeline و تغییرات انجام‌شده انجام دهید. اطمینان حاصل کنید که coherence issue اولیه (ناسازگاری منطقی) به طور کامل رفع شده است. مستندات نهایی شامل شرح issue اولیه، تغییرات انجام‌شده، و نحوه رفع ناسازگاری را تهیه کنید. همچنین اطمینان حاصل کنید که هیچ فرض ناسازگار دیگری در کد باقی نمانده است.
**Excerpt:**
```
