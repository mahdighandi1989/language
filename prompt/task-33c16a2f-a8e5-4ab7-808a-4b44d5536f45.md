---
task_id: 33c16a2f-a8e5-4ab7-808a-4b44d5536f45
title: اعتبارسنجی حجم فایل تلگرام پس از دانلود کامل انجام می‌شود و edge case file.size غایب پوشش داده نشده
type: bug
priority: medium
...
created_at: '2026-06-13T08:12:02.768739+00:00'
updated_at: '2026-06-13T08:19:47.898214+00:00'
target_files:
- backend/services/telegram/commands.js
---

# اعتبارسنجی حجم فایل تلگرام پس از دانلود کامل انجام می‌شود و edge case file.size غایب پوشش داده نشده

## Raw Idea

در backend/services/telegram/commands.js متد _uploadReceive حجم را با if (file.size && file.size > this.config.maxFileBytes) چک می‌کند. مشکل: اگر file.size تعریف نشده باشد (برخی انواع آپدیت تلگرام size را همراه ندارند) شرط false می‌شود و فایل بدون محدودیت پذیرفته می‌شود — یک edge case ورودی null/undefined که می‌تواند منجر به مصرف بیش از حد منابع یا خطای پایین‌دستی شود. همچنین پیام خطا حجم ثابت «۲۰MB» را hard-code کرده در حالی که cmdUpload از this.config.maxFileBytes استفاده می‌کند (ناسازگاری پیام).
...
- `backend/services/telegram/commands.js:390-396` — `_uploadReceive` — file.size && ... باعث عبور فایل بدون size می‌شود؛ پیام hard-code
  ```jsx
...
Telegram bot + in-memory store

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/services/telegram/config.js` — maxFileBytes از اینجا می‌آید
- `backend/services/telegram/store.js` — addContent پس از این بررسی صدا زده می‌شود
- `backend/services/telegram/practice.js` — `commands.js` این فایل را import می‌کند
- `backend/services/telegram/index.js` — این فایل `commands.js` را import می‌کند (caller)
- `backend/tests/test_telegram_commands.js` — این فایل `commands.js` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
commands.js از index.js و دو تست تلگرام مصرف می‌شود؛ اصلاح بررسی size فقط بر مسیر upload اثر دارد.
...
- [ ] آپلود فایل بدون فیلد size با پیام خطای مشخص رد می‌شود
- [ ] پیام خطای حجم از this.config.maxFileBytes مشتق می‌شود نه عدد ثابت
- [ ] تست واحد برای فایل بدون size و فایل بزرگ‌
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. وقتی file.size موجود نیست، یا فایل را رد کن (پیام خطای واضح) یا پس از دریافت اندازهٔ واقعی را بررسی کن. پیام خطا را به استفاده از Math.round(this.config.maxFileBytes/1024/1024) تغییر بده تا با cmdUpload یکدست شود.

## 💡 نمونه‌های قبل/بعد
**بررسی صریح وجود size**

_قبل:_
```
if (file.size && file.size > this.config.maxFileBytes) {
  return ctx.reply('... (۲۰MB).');
}
```

_بعد:_
```
const limitMB = Math.round(this.config.maxFileBytes / 1024 / 1024);
if (file.size == null) {
  return ctx.reply('حجم فایل قابل تشخیص نیست؛ دوباره ارسال کنید.');
}
if (file.size > this.config.maxFileBytes) {
  return ctx.reply(`حجم فایل بیشتر از حد مجاز است (${limitMB}MB).`);
}
```
...
1. آپلود فایل بدون فیلد size با پیام خطای مشخص رد می‌شود _(verify: backend_test)_
2. پیام خطای حجم از this.config.maxFileBytes مشتق می‌شود نه عدد ثابت _(verify: backend_test)_
3. تست واحد برای فایل بدون size و فایل بزرگ‌ _(verify: backend_test)_
