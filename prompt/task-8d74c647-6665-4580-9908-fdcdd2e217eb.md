---
task_id: 8d74c647-6665-4580-9908-fdcdd2e217eb
title: خطای async در attachTelegram بدون await/catch روی startTelegram باعث UnhandledPromiseRejection می‌شود
type: bug
priority: high
...
created_at: '2026-06-13T08:10:56.570138+00:00'
updated_at: '2026-06-13T08:17:34.254936+00:00'
target_files:
- backend/services/telegram/index.js
- backend/services/telegram/index.js
---

# خطای async در attachTelegram بدون await/catch روی startTelegram باعث UnhandledPromiseRejection می‌شود

## Raw Idea

در backend/services/telegram/index.js تابع attachTelegram (همگام) متد startTelegram(service) را بدون await و بدون .catch() فراخوانی می‌کند. اگرچه startTelegram خودش یک try/catch داخلی دارد که فقط setupWebhook/startPolling را پوشش می‌دهد، اما اگر هر مرحلهٔ قبل از try (مثلاً دسترسی به service.config.mode وقتی config ناقص است) یا خود service.logger.recordError throw کند، یک promise rejection بدون هندلر باقی می‌ماند. در server.js این تابع داخل attachTelegram(app, {...}) به‌صورت همگام صدا زده می‌شود و هیچ‌کس rejection حاصل را نمی‌گیرد. این در failure mode بوت سرور (token پیکربندی‌شده ولی شبکهٔ تلگرام در دسترس نیست) خطرناک است.
...
- `backend/services/telegram/index.js:60-73` — `startTelegram` — config access قبل از try نیست اما فراخوانی بدون catch در attachTelegram مشکل اصلی است
  ```jsx
  ...
  ```
- `backend/services/telegram/index.js:126` — `attachTelegram` — promise رها شده بدون catch
  ```jsx
...
Node/Express ESM + ws + Telegram bot polling/webhook

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/server.js` (سطر 87) — attachTelegram را در composition root صدا می‌زند
- `backend/services/telegram/bot.js` — setupWebhook/startPolling که ممکن است reject کنند
...
## 🌐 نقشهٔ وابستگی‌ها
attachTelegram تنها از server.js صدا زده می‌شود؛ تغییر آن فقط روی مسیر بوت سرور اثر می‌گذارد و ریسک پایینی دارد.
...
- [ ] اگر startTelegram reject شود، خطا از طریق logger.recordError ثبت می‌شود و process.on('unhandledRejection') فعال نمی‌شود
- [ ] تست واحد جدید که با bot ساختگی throw می‌کند تأیید کند هیچ unhandled rejection رخ نمی‌دهد
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. در attachTelegram فراخوانی startTelegram را به صورت startTelegram(service).catch((err) => logger.recordError('attachTelegram:start', err)); تغییر بده تا هر rejection غیرمنتظره صریحاً log شود. همچنین در startTelegram کل بدنه (شامل دسترسی به service.config) را داخل try قرار بده.

## 💡 نمونه‌های قبل/بعد
**catch صریح روی promise رها‌شده**
...
_بعد:_
```
startTelegram(service).catch((err) => logger.recordError('attachTelegram:start', err));
  return service;
```
...
کم؛ صرفاً افزودن catch. رفتار موفق بدون تغییر می‌ماند.
...
1. اگر startTelegram reject شود، خطا از طریق logger.recordError ثبت می‌شود و process.on('unhandledRejection') فعال نمی‌شود _(verify: backend_test)_
2. تست واحد جدید که با bot ساختگی throw می‌کند تأیید کند هیچ unhandled rejection رخ نمی‌دهد _(verify: backend_test)_
