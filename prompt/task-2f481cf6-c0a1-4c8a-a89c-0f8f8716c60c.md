---
task_id: 2f481cf6-c0a1-4c8a-a89c-0f8f8716c60c
title: مسیر بحرانی امنیتی security.js (CORS→403 translator + CSP) فایل تست واحد ندارد
type: test
priority: high
...
created_at: '2026-06-13T08:10:56.495104+00:00'
updated_at: '2026-06-13T08:17:06.663711+00:00'
---

# مسیر بحرانی امنیتی security.js (CORS→403 translator + CSP) فایل تست واحد ندارد

## Raw Idea

backend/middleware/security.js دو نگرانی امنیتی را owns می‌کند: ساخت allow-list مبدأ CORS (buildAllowedOrigins) و یک error-handling middleware که خطای 'Not allowed by CORS' را به پاسخ 403 JSON ترجمه می‌کند، به‌علاوهٔ تنظیم Content-Security-Policy. علی‌رغم اینکه backend/app/main.py یک ground-truth contract برای error_handling (ERROR_HANDLING_CONTRACT و EXPECTED_ERROR_MIDDLEWARES=2) دارد و این مسیر امنیتی است، هیچ تست واحد JS مستقیمی برای buildAllowedOrigins یا برای صحت ترجمهٔ 403 وجود ندارد. این یک critical path (security/CORS) است. شواهد: applySecurity (خطوط 35
