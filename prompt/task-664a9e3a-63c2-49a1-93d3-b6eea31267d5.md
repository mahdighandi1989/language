---
task_id: 664a9e3a-63c2-49a1-93d3-b6eea31267d5
title: '[منطق] Telegram Service Auth Not Covered by Firebase Verification Scope'
type: logic_audit
priority: high
...
created_at: '2026-06-06T07:56:39.166562+00:00'
updated_at: '2026-06-06T08:03:06.552863+00:00'
---

# [منطق] Telegram Service Auth Not Covered by Firebase Verification Scope
...
The system includes `backend/tests/test_firebase_auth.py` specifically designed to verify Firebase ID token authentication on 'backend Gemini API endpoints'. However, `backend/services/telegram/practice.js`, which orchestrates AI teacher practice and interacts with an `aiProvider` (potentially Gemini), is a critical backend service handling user interactions and resource consumption. Its description does not indicate that it uses Firebase ID token authentication, nor is it explicitly mentioned a

## 💥 پیامد (impact)
The current testing strategy provides a false sense of security by focusing only on Firebase-protected endpoints, leaving the Telegram service's authentication and authorization mechanisms unverified. This can lead to unidentified vulnerabilities in a critical user-facing component and architectural

## 🛠 پیشنهاد رفع اولیه
Clarify the authentication and authorization strategy for `backend/services/telegram/practice.js`. If it's meant to use Firebase auth, update `practice.js` to reflect this and extend `test_firebase_auth.py` to cover its entry points. If a separate mechanism is used, create a new dedicated test component to verify its correct implementation and enforcement.
...
### Step 1: بررسی و شفاف‌سازی استراتژی احراز هویت برای سرویس Telegram Practice
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی و شفاف‌سازی کامل استراتژی احراز هویت و مجوزدهی (authentication and authorization) برای سرویس `backend/services/telegram/practice.js` است. هدف این است که مشخص شود آیا این سرویس قرار است از احراز هویت Firebase ID token استفاده کند یا مکانیزم جداگانه‌ای برای آن در نظر گرفته شده است. این مرحله شامل تحلیل تعاملات `practice.js` با `aiProvider` و نحوه مدیریت تعاملات کاربر و مصرف منابع است تا تصمیم‌گیری نهایی در مورد استراتژی احراز هویت انجام شود. این مرحله صرفاً تحقیقاتی و تصمیم‌گیری است و هیچ تغییر کدی در آن انجام نمی‌شود.
**Excerpt:**
```
Clarify the authentication and authorization strategy for `backend/services/telegram/practice.js`. If it's meant to use Firebase auth, update `practice.js` to reflect this and extend `test_firebase_auth.py` to cover its entry points. If a separate mechanism is used, create a new dedicated test component to verify its correct implementation and enforcement.
```

### Step 2: پیاده‌سازی احراز هویت Firebase در `backend/services/telegram/practice.js` (مشروط)
**Status:** `pending` (0%)
**Scope:** این مرحله **مشروط** است و تنها در صورتی اجرا می‌شود که مرحله ۱ (شفاف‌سازی استراتژی) به این نتیجه برسد که `backend/services/telegram/practice.js` باید از احراز هویت Firebase ID token استفاده کند. در این مرحله، فایل `backend/services/telegram/practice.js` به‌روزرسانی می‌شود تا مکانیزم‌های احراز هویت Firebase را در خود جای دهد. این شامل افزودن منطق برای اعتبارسنجی توکن‌های Firebase ID دریافتی در درخواست‌ها یا تعاملات مربوط به این سرویس است تا اطمینان حاصل شود که فقط کاربران احراز هویت شده می‌توانند با آن تعامل داشته باشند.
**Excerpt:**
```
If it's meant to use Firebase auth, update `practice.js` to reflect this and extend `test_firebase_auth.py` to cover its entry points.
```

### Step 3: گسترش `backend/tests/test_firebase_auth.py` برای پوشش `practice.js` (مشروط)
**Status:** `pending` (0%)
**Scope:** این مرحله **مشروط** است و تنها در صورتی اجرا می‌شود که مرحله ۲ (پیاده‌سازی احراز هویت Firebase در `practice.js`) انجام شده باشد. در این مرحله، تست‌های جدیدی به فایل `backend/tests/test_firebase_auth.py` اضافه می‌شود که به‌طور خاص برای تأیید احراز هویت Firebase ID token برای نقاط ورودی (entry points) سرویس `backend/services/telegram/practice.js` طراحی شده‌اند. این تست‌ها باید سناریوهای احراز هویت موفق، احراز هویت ناموفق (مانند توکن‌های نامعتبر یا منقضی شده) و موارد خاص مربوط به تعاملات `practice.js` را پوشش دهند.
**Excerpt:**
```
...and extend `test_firebase_auth.py` to cover its entry points.
```

### Step 4: ایجاد کامپوننت تست اختصاصی برای مکانیزم احراز هویت جداگانه `backend/services/telegram/practice.js` (مشروط)
**Status:** `pending` (0%)
**Scope:** این مرحله **مشروط** است و تنها در صورتی اجرا می‌شود که مرحله ۱ (شفاف‌سازی استراتژی) به این نتیجه برسد که `backend/services/telegram/practice.js` از یک مکانیزم احراز هویت و مجوزدهی *جداگانه* (غیر از Firebase ID token) استفاده می‌کند. در این مرحله، یک فایل تست جدید و اختصاصی (مثلاً `backend/tests/test_telegram_practice_custom_auth.py` یا نامی مشابه) به همراه موارد تست مربوطه ایجاد می‌شود. این تست‌ها باید به‌طور کامل پیاده‌سازی و اعمال صحیح این مکانیزم احراز هویت خاص را در `backend/services/telegram/practice.js` تأیید کنند و اطمینان حاصل شود که اقدامات امنیتی سفارشی یا جایگزین، قوی و بدون نقص هستند.
**Excerpt:**
```
If a separate mechanism is used, create a new dedicated test component to verify its correct implementation and enforcement.
```
