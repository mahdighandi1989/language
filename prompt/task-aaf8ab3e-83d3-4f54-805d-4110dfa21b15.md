---
task_id: aaf8ab3e-83d3-4f54-805d-4110dfa21b15
title: آپلود فایل بدون cleanup تضمینی منجر به انباشت فایل‌های موقت در /api/upload و /api/audio/process می‌شود
type: bug
priority: high
...
created_at: '2026-06-06T07:55:56.635899+00:00'
updated_at: '2026-06-06T08:01:48.364101+00:00'
target_files:
- backend/routes/index.js
- backend/middleware/upload.js
---

# آپلود فایل بدون cleanup تضمینی منجر به انباشت فایل‌های موقت در /api/upload و /api/audio/process می‌شود

## Raw Idea

مسیرهای POST /api/upload و POST /api/audio/process با upload.single('file') فایل را روی دیسک (uploadDir در os.tmpdir()) ذخیره می‌کنند اما برخلاف analysisService (که cleanupTempFiles را در finally صدا می‌زند) هیچ تضمینی برای حذف فایل پس از پردازش وجود ندارد. در analysisService.js نیز در مسیر بزرگ ویدئو cleanup داریم اما فایل‌های آپلودی اصلی (file.path) که توسط multer روی دیسک نوشته شده‌اند باید همگی پاک شوند؛ مسیر upload فقط handle برمی‌گرداند و فایل را روی دیسک رها می‌کند. هر فایل تا 500MB است (MAX_FILE_SIZE) و در production این به پر شدن دیسک و failure سرویس می‌انجامد. این یک failure mode تحت بار عادی است.
...
- `backend/routes/index.js:65-82` — `apiRouter.post('/api/upload') / apiRouter.post('/api/audio/process')` — این مسیرها فایل را روی دیسک می‌نویسند ولی هیچ cleanup در زنجیره دیده نمی‌شود
  ```jsx
  apiRouter.post(
    '/api/upload',
    upload.single('file'),
  ...
  ```
- `backend/middleware/upload.js:9-17` — `uploadDir / storage` — محل ذخیرهٔ فایل‌ها روی دیسک که باید پاک شوند
  ```jsx
...
Node/Express + multer diskStorage + ffmpeg + os.tmpdir()

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/utils/fileCleanup.js` — تابع cleanupTempFiles که باید در این مسیرها فراخوانی شود
- `backend/services/analysisService.js` (سطر 14) — مرجع صحیح: cleanupTempFiles را در finally صدا می‌زند
- `backend/controllers/uploadController.js` — controller که فایل را پردازش می‌کند و باید cleanup بزند
- `backend/controllers/audioController.js` — controller پردازش صوت که فایل موقت ایجاد می‌کند
- `backend/middleware/validate.js` — `index.js` این فایل را import می‌کند
- `backend/middleware/firebaseAuth.js` — `index.js` این فایل را import می‌کند
- `backend/middleware/requireGeminiKey.js` — `index.js` این فایل را import می‌کند
- `backend/middleware/rateLimiter.js` — `index.js` این فایل را import می‌کند
- `backend/server.js` — این فایل `index.js` را import می‌کند (caller)
- `backend/routes/index.js` — این فایل `upload.js` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
uploadDir از middleware/upload.js توسط routes و analysisService استفاده می‌شود؛ نبود cleanup در دو مسیر آپلود مستقیماً روی پایداری disk در production اثر می‌گذارد.
...
- [ ] پس از POST /api/upload فایل موقت در uploadDir باقی نمی‌ماند (تست با شمارش فایل‌ها قبل/بعد)
- [ ] پس از POST /api/audio/process هر فایل ورودی و خروجی transcode پاک می‌شود
- [ ] یک job/تابع پاک‌سازی فایل‌های قدیمی‌تر از TTL در uploadDir وجود دارد و تست واحد دارد
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. در uploadController و audioController پس از پردازش، فایل موقت (req.file?.path) را در یک بلاک finally با fs.unlink/cleanupTempFiles حذف کن. همچنین یک job دوره‌ای (مثل fileCleanup با TTL) برای پاک‌سازی فایل‌های یتیم در uploadDir اضافه کن تا فایل‌هایی که در صورت crash باقی می‌مانند نیز جمع شوند.
...
export async function uploadFile(req, res) {
  try {
    ...
    if (req.file?.path) await cleanupTempFiles([req.file.path]);
  }
}
```
...
## 🧪 دستورات اعتبارسنجی
- `node --test backend/tests/test_upload_validation.js`
- `python -m pytest tests/test_audio_processing.py`

## ⚠️ ریسک‌ها و موارد احتیاط
اگر cleanup قبل از پاسخ async به‌اشتباه زود اجرا شود ممکن است فایل لازم را حذف کند؛ باید ترتیب پردازش/پاسخ حفظ شود.
...
1. پس از POST /api/upload فایل موقت در uploadDir باقی نمی‌ماند (تست با شمارش فایل‌ها قبل/بعد) _(verify: backend_test)_
2. پس از POST /api/audio/process هر فایل ورودی و خروجی transcode پاک می‌شود _(verify: backend_test)_
3. یک job/تابع پاک‌سازی فایل‌های قدیمی‌تر از TTL در uploadDir وجود دارد و تست واحد دارد _(verify: backend_test)_
