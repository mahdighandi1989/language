---
task_id: 799dd6e8-3266-425b-b475-407a38c8957e
title: '[منطق] Ambiguity in `event` to `caption` mapping and `notify_event` concept'
type: logic_audit
priority: low
...
created_at: '2026-06-06T07:59:30.276223+00:00'
updated_at: '2026-06-06T07:59:30.276226+00:00'
---

# [منطق] Ambiguity in `event` to `caption` mapping and `notify_event` concept
...
The prompt's coherence check explicitly mentions `event` → `notify_event` → `caption`. While `backend/app/notifications.py` takes an `event (str)` and `context (Dict)`, the term `caption` is not explicitly defined as an input or output in any component description. This suggests an implicit mapping or a missing detail on how the `event` and `context` are transformed into the final message content (caption) for Telegram. Additionally, `notify_event` is used as a step in the flow, but its concrete

## 💥 پیامد (impact)
Lack of clarity on how notification messages are constructed. Developers might struggle to understand how to format `event` and `context` to produce desired Telegram messages, leading to inconsistent, uninformative, or poorly formatted alerts. The absence of `caption` definition indicates a potentia

## 🛠 پیشنهاد رفع اولیه
Explicitly define how the `event` string and `context` dictionary are used within `backend/app/notifications.py` to construct the final message (caption) for Telegram. If `caption` is an internal concept, it should be documented. If `notify_event` refers to a specific function, it should be named (e.g., `send_notification`).
...
### Step 1: تعریف صریح نحوه ساخت پیام (caption) از event و context
**Status:** `pending` (0%)
**Scope:** این مرحله بر تعریف و پیاده‌سازی منطق تبدیل `event` (رشته) و `context` (دیکشنری) به محتوای نهایی پیام (که در اینجا `caption` نامیده شده) برای تلگرام در فایل `backend/app/notifications.py` تمرکز دارد. هدف، شفاف‌سازی فرآیند ساخت پیام و اطمینان از سازگاری آن است. این مرحله شامل مستندسازی داخلی `caption` یا تغییر نام توابع نیست، بلکه فقط به منطق ساخت پیام می‌پردازد.
**Excerpt:**
```
Explicitly define how the `event` string and `context` dictionary are used within `backend/app/notifications.py` to construct the final message (caption) for Telegram. While `backend/app/notifications.py` takes an `event (str)` and `context (Dict)`, the term `caption` is not explicitly defined as an input or output in any component description. This suggests an implicit mapping or a missing detail on how the `event` and `context` are transformed into the final message content (caption) for Telegram.
```

### Step 2: مستندسازی مفهوم داخلی caption
**Status:** `pending` (0%)
**Scope:** این مرحله بر مستندسازی مفهوم `caption` در صورتی که یک مفهوم داخلی در `backend/app/notifications.py` باشد، تمرکز دارد. هدف، شفاف‌سازی نقش `caption` در فرآیند تولید پیام و نحوه ارتباط آن با `event` و `context` است. این مستندسازی باید در کنار کد مربوطه یا در فایل‌های مستندات پروژه انجام شود تا توسعه‌دهندگان آینده به راحتی بتوانند آن را درک کنند.
**Excerpt:**
```
If `caption` is an internal concept, it should be documented. The prompt's coherence check explicitly mentions `event` → `notify_event` → `caption`. While `backend/app/notifications.py` takes an `event (str)` and `context (Dict)`, the term `caption` is not explicitly defined as an input or output in any component description.
```

### Step 3: نام‌گذاری صریح تابع notify_event به send_notification
**Status:** `pending` (0%)
**Scope:** این مرحله بر شناسایی و نام‌گذاری صریح تابع یا منطقی که به عنوان `notify_event` در جریان کار شناخته می‌شود، تمرکز دارد. پیشنهاد می‌شود که این تابع به `send_notification` تغییر نام یابد تا هدف آن (ارسال نوتیفیکیشن) به وضوح مشخص شود. این تغییر شامل به‌روزرسانی تمام فراخوانی‌ها و ارجاعات به این تابع در سراسر codebase است تا از بروز خطاهای broken reference جلوگیری شود.
**Excerpt:**
```
If `notify_event` refers to a specific function, it should be named (e.g., `send_notification`). Additionally, `notify_event` is used as a step in the flow, but its concrete definition is missing.
```
