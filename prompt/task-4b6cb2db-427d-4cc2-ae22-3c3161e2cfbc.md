---
task_id: 4b6cb2db-427d-4cc2-ae22-3c3161e2cfbc
title: استفاده از وایلدکارد گسترده در Content Security Policy (CSP)
type: refactor
priority: medium
...
# استفاده از وایلدکارد گسترده در Content Security Policy (CSP)

## Raw Idea

در `backend/middleware/security.js`، خط‌مشی Content Security Policy (CSP) از وایلدکارد‌های گسترده‌ای مانند `https://*.googleapis.com`، `wss://*.googleapis.com`، `https://*.firebaseapp.com` و `https://*.firebaseio.com` برای `connect-src` و `frame-src` استفاده می‌کند. اگرچه این دامنه‌ها برای عملکرد Firebase و Google Gemini ضروری هستند، اما استفاده از وایلدکارد گسترده می‌تواند سطح حمله را افزایش دهد. در صورت به خطر افتادن یک زیردامنه از این سرویس‌های مورد اعتماد، ممکن است CSP دور زده شود.
...
- `backend/middleware/security.js:40-51` — `applySecurity` — تعریف خط‌مشی Content-Security-Policy
  ```jsx
  res.setHeader(
...
Node.js/Express backend، React/Vite frontend، Firebase، Google Gemini API.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/server.js` (سطر 84) — این فایل تابع `applySecurity` را فراخوانی می‌کند که CSP را تنظیم می‌کند.
- `frontend/index.html` (سطر 1) — صفحهٔ اصلی فرانت‌اند که تحت تأثیر CSP قرار می‌گیرد.

## 🌐 نقشهٔ وابستگی‌ها
این تغییر بر تمام ارتباطات فرانت‌اند با سرویس‌های خارجی (Firebase، Google API) تأثیر می‌گذارد و نیازمند تست دقیق برای جلوگیری از شکستن عملکرد برنامه است.
...
- [ ] CSP فقط دامنه‌های دقیق مورد نیاز را مجاز کند.
- [ ] عملکرد Firebase Auth و Gemini API بدون مشکل باقی بماند.
- [ ] تست‌های E2E برای اطمینان از عدم شکستن ارتباطات اضافه شود.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. خط‌مشی CSP باید بازبینی و سخت‌گیرانه‌تر شود. در صورت امکان، به جای وایلدکارد‌های گسترده، دامنه‌های دقیق و مشخصی که برنامه واقعاً به آن‌ها نیاز دارد، لیست شوند. همچنین می‌توان از `nonce` یا `hash` برای اسکریپت‌ها استفاده کرد تا امنیت بیشتری فراهم شود.

## 💡 نمونه‌های قبل/بعد
**نمونهٔ سخت‌گیرانه‌تر کردن CSP**
...
_بعد:_
```
connect-src 'self' blob: data: https://generativelanguage.googleapis.com wss://generativelanguage.googleapis.com https://auth.firebase.com https://firestore.googleapis.com ...
```
...
## 🧪 دستورات اعتبارسنجی
- `npm run dev`
- `بررسی کنسول مرورگر برای خطاهای CSP`

## ⚠️ ریسک‌ها و موارد احتیاط
احتمال مسدود شدن منابع مشروع و شکستن عملکرد برنامه در صورت پیکربندی نادرست CSP.
