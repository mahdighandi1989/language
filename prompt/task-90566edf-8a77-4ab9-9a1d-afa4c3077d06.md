...
updated_at: '2026-06-06T07:57:45.014315+00:00'
target_files:
...
1. mountFallbacks یک middleware با امضای (err,req,res,next) ثبت می‌کند که res.headersSent را گارد می‌کند
2. هر خطای forward شده با body JSON یکنواخت و status پیش‌فرض 500 پاسخ داده می‌شود
3. یک تست JS شمار middlewareهای error-handling را با EXPECTED_ERROR_MIDDLEWARES=2 تطبیق می‌دهد
