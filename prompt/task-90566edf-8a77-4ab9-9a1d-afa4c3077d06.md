---
task_id: 90566edf-8a77-4ab9-9a1d-afa4c3077d06
title: 'ناسازگاری ground-truth: قرارداد ادعا می‌کند EXPECTED_ERROR_MIDDLEWARES=2 اما security.js فقط CORS translator را ثبت می‌کند و terminal handler در fallbackController است بدون verify خودکار'
type: bug
priority: medium
...
created_at: '2026-06-06T07:57:45.014312+00:00'
updated_at: '2026-06-06T08:04:36.139197+00:00'
target_files:
...
# ناسازگاری ground-truth: قرارداد ادعا می‌کند EXPECTED_ERROR_MIDDLEWARES=2 اما security.js فقط CORS translator را ثبت می‌کند و terminal handler در fallbackController است بدون verify خودکار

## Raw Idea

در backend/app/main.py مقدار EXPECTED_ERROR_MIDDLEWARES = 2 و قرارداد ERROR_HANDLING_CONTRACT می‌گوید terminal handler با scope 'app level' و registered_after 'applySecurity' باید وجود داشته باشد. اما applySecurity در security.js فقط یک middleware (err, req, res, next) برای CORS ثبت می‌کند (خط 56-61) و next(err) را فراخوانی می‌کند. اگر mountFallbacks در fallbackController به‌درستی یک catch-all terminal handler ثبت نکند، خطاهای forward شده هیچ‌جا گرفته نمی‌شوند و response shape بسته به branch متفاوت می‌شود — همان anti-pattern که قرارداد ادعا می‌کند حل شده است. fallbackController.js در نمونه‌های ارائه‌شده موجود نیست تا تأیید شود این invariant واقعاً برقرار است (نیاز به تأیید). همچنین هیچ تست runtime که شمارش واقعی middlewareهای (err,req,res,next) را با EXPECTED_ERROR_MIDDLEWARES تطبیق دهد در Node side دیده نمی‌شود.
...
- `backend/middleware/security.js:55-61` — `applySecurity (CORS translator)` — این فقط CORS را ترجمه می‌کند و خطاهای دیگر را forward می‌کند؛ terminal handler جداست
  ```jsx
  ...
  ```
- `backend/app/main.py:55-57` — `EXPECTED_ERROR_MIDDLEWARES`
  ```python
  EXPECTED_ERROR_MIDDLEWARES = 2
  ```
- `backend/server.js:107-108` — `app.use(apiRouter) / mountFallbacks(app)` — نقطهٔ wiring که terminal handler باید ثبت شود
  ```jsx
  app.use(apiRouter);
...
Express error-handling middleware (4-arity) + Python ground-truth mirror

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/controllers/fallbackController.js` — terminal error handler اینجا تعریف می‌شود و باید verify شود
- `tests/test_error_handling.py` — تست edge-case که قرارداد را assert می‌کند
- `tests/test_anti_pattern_edge_case.py` — تست anti-pattern conditional inconsistency
- `backend/config/validateEnv.js` — `server.js` این فایل را import می‌کند
- `backend/config/bootstrap.js` — `server.js` این فایل را import می‌کند
- `backend/config/env.js` — `server.js` این فایل را import می‌کند
- `backend/controllers/index.js` — `server.js` این فایل را import می‌کند

## 🌐 نقشهٔ وابستگی‌ها
applySecurity و mountFallbacks هر دو در server.js wire می‌شوند؛ صحت این invariant روی یکنواختی پاسخ خطا در کل API اثر می‌گذارد.
...
- [ ] mountFallbacks یک middleware با امضای (err,req,res,next) ثبت می‌کند که res.headersSent را گارد می‌کند
- [ ] هر خطای forward شده با body JSON یکنواخت و status پیش‌فرض 500 پاسخ داده می‌شود
- [ ] یک تست JS شمار middlewareهای error-handling را با EXPECTED_ERROR_MIDDLEWARES=2 تطبیق می‌دهد
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. محتوای backend/controllers/fallbackController.js را بررسی کن و مطمئن شو mountFallbacks یک terminal error_handling_middleware با امضای (err, req, res, next) ثبت می‌کند که res.headersSent را گارد می‌کند و پاسخ JSON یکنواخت با default_status=500 برمی‌گرداند. سپس یک تست JS اضافه کن که شمار middlewareهای error-handling ثبت‌شده در app را بشمارد و با EXPECTED_ERROR_MIDDLEWARES=2 برابر باشد، تا قرارداد پایتونی صرفاً مستند نباشد بلکه توسط runtime اعمال شود.

## 💡 نمونه‌های قبل/بعد
**تست runtime شمارش middleware**

_قبل:_
```
// no runtime assertion of middleware count
```

_بعد:_
```
const errMw = app._router.stack.filter(l => l.handle.length === 4);
expect(errMw.length).toBe(EXPECTED_ERROR_MIDDLEWARES); // 2
```
...
## 🧪 دستورات اعتبارسنجی
- `python -m pytest tests/test_error_handling.py tests/test_anti_pattern_edge_case.py`
- `node --test backend/tests/test_validateEnv.js`

## ⚠️ ریسک‌ها و موارد احتیاط
اگر fallbackController از قبل صحیح باشد این صرفاً افزودن تست است؛ ریسک پایین.
...
1. mountFallbacks یک middleware با امضای (err,req,res,next) ثبت می‌کند که res.headersSent را گارد می‌کند _(verify: static)_
2. هر خطای forward شده با body JSON یکنواخت و status پیش‌فرض 500 پاسخ داده می‌شود _(verify: api_response)_
3. یک تست JS شمار middlewareهای error-handling را با EXPECTED_ERROR_MIDDLEWARES=2 تطبیق می‌دهد _(verify: backend_test)_

## Task Steps

### Step 1: بررسی و تأیید مقادیر EXPECTED_ERROR_MIDDLEWARES و ERROR_HANDLING_CONTRACT
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی فایل `backend/app/main.py` برای تأیید وجود و مقادیر `EXPECTED_ERROR_MIDDLEWARES` (که انتظار می‌رود ۲ باشد) و تعریف `ERROR_HANDLING_CONTRACT` است. هدف این است که اطمینان حاصل شود که قرارداد و انتظارات مربوط به تعداد middlewareها به درستی در کد پایه تعریف شده‌اند. این مرحله شامل هیچ تغییری در کد نیست، بلکه صرفاً یک مرحله تأیید و درک وضعیت موجود است.
**Excerpt:**
```
در backend/app/main.py مقدار EXPECTED_ERROR_MIDDLEWARES = 2 و قرارداد ERROR_HANDLING_CONTRACT می‌گوید terminal handler با scope 'app level' و registered_after 'applySecurity' باید وجود داشته باشد.
```

### Step 2: بررسی ثبت middleware توسط applySecurity در security.js
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی دقیق فایل `security.js`، به ویژه خطوط 56-61، برای تأیید اینکه تابع `applySecurity` دقیقاً چند middleware از نوع `(err, req, res, next)` را ثبت می‌کند. هدف این است که مشخص شود آیا `applySecurity` تنها یک middleware برای CORS ثبت می‌کند و سپس `next(err)` را فراخوانی می‌کند، که نشان‌دهنده عدم وجود یک terminal handler کامل در این بخش است. این مرحله صرفاً یک بررسی و تأیید وضعیت موجود است.
**Excerpt:**
```
اما applySecurity در security.js فقط یک middleware (err, req, res, next) برای CORS ثبت می‌کند (خط 56-61) و next(err) را فراخوانی می‌کند.
```

### Step 3: بررسی وجود و عملکرد mountFallbacks در fallbackController.js
**Status:** `pending` (0%)
**Scope:** این مرحله شامل جستجو برای فایل `fallbackController.js` و در صورت وجود، بررسی تابع `mountFallbacks` آن است. هدف این است که تأیید شود آیا `mountFallbacks` به درستی یک `catch-all terminal handler` را با `scope 'app level'` و `registered_after 'applySecurity'` ثبت می‌کند. اگر فایل یا تابع موجود نباشد یا به درستی عمل نکند، این مرحله این عدم انطباق را شناسایی می‌کند. این مرحله صرفاً یک بررسی و تأیید وضعیت موجود است.
**Excerpt:**
```
اگر mountFallbacks در fallbackController به‌درستی یک catch-all terminal handler ثبت نکند، خطاهای forward شده هیچ‌جا گرفته نمی‌شوند و response shape بسته به branch متفاوت می‌شود — همان anti-pattern که قرارداد ادعا می‌کند حل شده است. fallbackController.js در نمونه‌های ارائه‌شده موجود نیست تا تأیید شود این invariant واقعاً برقرار است (نیاز به تأیید).
```

### Step 4: پیاده‌سازی یا اصلاح mountFallbacks برای ثبت Catch-All Terminal Handler
**Status:** `pending` (0%)
**Scope:** این مرحله شامل ایجاد فایل `fallbackController.js` در صورت عدم وجود، یا اصلاح تابع `mountFallbacks` در آن است. هدف این است که اطمینان حاصل شود `mountFallbacks` یک `catch-all terminal handler` را به درستی ثبت می‌کند که با `ERROR_HANDLING_CONTRACT` مطابقت دارد، یعنی دارای `scope 'app level'` و `registered_after 'applySecurity'` باشد. این handler باید مسئول گرفتن تمام خطاهای forward شده باشد تا از `response shape`های متفاوت جلوگیری کند و `invariant` قرارداد را برقرار سازد.
**Excerpt:**
```
اگر mountFallbacks در fallbackController به‌درستی یک catch-all terminal handler ثبت نکند، خطاهای forward شده هیچ‌جا گرفته نمی‌شوند و response shape بسته به branch متفاوت می‌شود — همان anti-pattern که قرارداد ادعا می‌کند حل شده است. fallbackController.js در نمونه‌های ارائه‌شده موجود نیست تا تأیید شود این invariant واقعاً برقرار است (نیاز به تأیید).
```

### Step 5: افزودن تست runtime برای شمارش middlewareهای (err,req,res,next)
**Status:** `pending` (0%)
**Scope:** این مرحله شامل توسعه و اضافه کردن یک تست runtime جدید به پروژه Node.js است. این تست باید به صورت پویا تعداد واقعی middlewareهای از نوع `(err,req,res,next)` را که در برنامه ثبت شده‌اند، شمارش کند و این تعداد را با مقدار `EXPECTED_ERROR_MIDDLEWARES` که در `backend/app/main.py` تعریف شده است، مقایسه و تأیید کند. هدف این است که یک مکانیزم خودکار برای اطمینان از انطباق پیکربندی مدیریت خطا با انتظارات قرارداد فراهم شود.
**Excerpt:**
```
همچنین هیچ تست runtime که شمارش واقعی middlewareهای (err,req,res,next) را با EXPECTED_ERROR_MIDDLEWARES تطبیق دهد در Node side دیده نمی‌شود.
```
