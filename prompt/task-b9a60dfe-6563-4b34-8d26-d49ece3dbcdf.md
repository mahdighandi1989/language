---
task_id: b9a60dfe-6563-4b34-8d26-d49ece3dbcdf
...
updated_at: '2026-06-06T07:55:56.699743+00:00'
target_files:
...
The `boot_backend` function's Node.js probe uses a hardcoded `setTimeout(() => process.exit(0), 600)` to determine server readiness. This assumes 600ms is always sufficient for the server to reach a listening state after import. This is a brittle 'sleep-based synchronization' anti-pattern and a 'Threshold-Outcome mismatch'. If server initialization takes longer, the test will pass but the server m

📁 file: tests/_backend_helpers.py (line 55)

🎯 پیشنهاد: این الگو معمولاً منطق سیستم را در شرایط لبه می‌شکند.
---
[scan #2 at 2026-06-06T07:55:56.693982+00:00]
The `boot_backend` function starts the Node.js server on an ephemeral port (`PORT: '0'`) but does not extract or return the actual port number chosen by the operating system. This breaks the feedback loop, as subsequent test steps that need to interact with the backend will not know which port to co
---
...
- `tests/backend/test_server.py` — این فایل `_backend_helpers.py` را import می‌کند (caller)
- `tests/test_backend_startup.py` — این فایل `_backend_helpers.py` را import می‌کند (caller)
- `tests/test_server_startup.py` — این فایل `_backend_helpers.py` را import می‌کند (caller)
- `tests/test_websocket_error_handling.py` — این فایل `_backend_helpers.py` را import می‌کند (caller)
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile tests/_backend_helpers.py`
- `ruff check tests/_backend_helpers.py`
- `pytest -x`
