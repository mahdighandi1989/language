...
      <div className="h-[700px] flex flex-col bg-slate-100 rounded-xl p-3">
...
          <button onClick={() => fileInputRef.current.click()} className="p-3 border rounded-xl hover:bg-slate-200"><Paperclip size={20}/></button>
          <input type="file" ref={fileInputRef} onChange={handleFileAttach} className="hidden" />
          <input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleSend()} className="flex-1 p-3 border rounded-xl" placeholder="پیام خود را بنویسید..." disabled={isLoading} />
          <button onTouchStart={handleMicClick} onTouchEnd={handleMicClick} onMouseDown={handleMicClick} onMouseUp={handleMicClick} className={`p-3 border rounded-xl ${isRecording ? 'bg-red-500 text-white' : 'hover:bg-slate-200'}`}><Mic size={20}/></button>
...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg h-[80vh] flex flex-col" dir="rtl" onClick={e => e.stopPropagation()}>
