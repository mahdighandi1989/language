    ...
    <>
      ...
      <div className="h-[calc(100vh-200px)] min-h-[400px] max-h-[700px] flex flex-col bg-slate-100 rounded-xl p-2 sm:p-3">
        ...
        <div className="flex items-center gap-1 sm:gap-2 pt-2 flex-shrink-0">
          <button onClick={() => fileInputRef.current.click()} className="p-2 sm:p-3 border rounded-xl hover:bg-slate-200 flex-shrink-0"><Paperclip size={18}/></button>
          <input type="file" ref={fileInputRef} onChange={handleFileAttach} className="hidden" />
          <input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleSend()} className="flex-1 min-w-0 p-2 sm:p-3 border rounded-xl text-sm sm:text-base" placeholder="پیام خود را بنویسید..." disabled={isLoading} />
          <button onTouchStart={handleMicClick} onTouchEnd={handleMicClick} onMouseDown={handleMicClick} onMouseUp={handleMicClick} className={`p-2 sm:p-3 border rounded-xl flex-shrink-0 ${isRecording ? 'bg-red-500 text-white' : 'hover:bg-slate-200'}`}><Mic size={18}/></button>
          <button onClick={() => handleSend()} className="bg-teal-500 text-white px-3 sm:px-5 py-2 sm:py-3 rounded-xl hover:bg-teal-600 disabled:bg-slate-400 font-bold text-sm sm:text-base flex-shrink-0" disabled={isLoading}>{isLoading ? '...' : 'ارسال'}</button>
        </div>
      </div>
    </>
  );
